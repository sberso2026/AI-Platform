...
export function ProjectQueryDecisionIntelligenceView() {
  ...
  const [view, setView] = useState<QueryDecisionView | null>(null);
  ...
  const loadView = useCallback(async (projectId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/engineering/project-intelligence/projects/${encodeURIComponent(projectId)}/queries-decisions`,
      );
      const body = (await response.json()) as { data?: QueryDecisionView; error?: { message?: string } };
      if (!response.ok) {
        throw new Error(body.error?.message ?? `Queries & Decisions request failed (${response.status})`);
      }
      setView(body.data ?? null);
    } catch (err) {
      setView(null);
      setError(err instanceof Error ? err.message : "Query & Decision Intelligence unavailable");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div data-testid="project-intelligence-queries-decisions" className="space-y-8">
      ...
      {loading ? <p className="text-sm text-slate-600">Loading Query & Decision Intelligence…</p> : null}
      {error ? (
        <EmptyState title="Queries & Decisions unavailable" description={error} data-testid="queries-decisions-error" />
      ) : null}
