/**
 * Minimum Project Controls Assurance Intelligence HTTP API (Phase 11K).
 *
 * Advisory intelligence signals only; never mutates assurance registers,
...
const OPERATIONS = new Set(["assess_assurance", "review", "publish"]);
...
const GOVERNANCE_FLAGS = {
  ...
  assuranceIntelligenceOwnership: "project_controls",
...
export async function POST(req: Request) {
  ...
  const operation = typeof body.operation === "string" ? body.operation : "assess_assurance";
  ...
  const assuranceUnitId =
    typeof controlContext.assuranceUnitId === "string" ? controlContext.assuranceUnitId : "";
  if (
    (operation === "assess_assurance" || operation === "assess_assurance") &&
    !assuranceUnitId
  ) {
    return err(
      400,
      "missing_assurance_unit",
      "controlContext.assuranceUnitId is required",
      requestId,
    );
  }
  ...
  if (
    ...
    return err(
      422,
      "assurance_certification_forbidden",
      "Project Controls assesses advisory assurance intelligence and may not execute or select preferred scenarios",
      requestId,
    );
  }

  if (operation === "review" || operation === "publish") {
    const assuranceStateId =
      typeof body.assuranceStateId === "string" ? body.assuranceStateId : undefined;
    if (!assuranceStateId) {
      return err(400, "missing_state_id", "assuranceStateId is required", requestId);
    }
