/**
 * Minimum Project Controls Scenario Intelligence HTTP API (Phase 11I).
 *
 * Exploratory scenario comparison only; never auto-executes scenarios,
...
const OPERATIONS = new Set(["assess_scenario", "compare_scenarios", "review", "publish"]);
...
export async function POST(req: Request) {
  ...
  const operation = typeof body.operation === "string" ? body.operation : "assess_scenario";
  ...
  const scenarioUnitId =
    typeof controlContext.scenarioUnitId === "string" ? controlContext.scenarioUnitId : "";
  if (
    (operation === "assess_scenario" || operation === "compare_scenarios") &&
    !scenarioUnitId
  ) {
    return err(
      400,
      "missing_scenario_unit",
      "controlContext.scenarioUnitId is required",
      requestId,
    );
  }
  ...
  if (operation === "review" || operation === "publish") {
    const scenarioStateId =
      typeof body.scenarioStateId === "string" ? body.scenarioStateId : undefined;
    if (!scenarioStateId) {
      return err(400, "missing_state_id", "scenarioStateId is required", requestId);
    }
