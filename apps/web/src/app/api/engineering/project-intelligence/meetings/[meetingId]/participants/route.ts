...
import { addParticipant, listParticipants } from "@/lib/project-intelligence/meetings-service";
...
export const GET = withEngineeringApiParams<{ meetingId: string }>(
  ...
  async (context, _request, params) => {
    try {
      ...
      const data = await listParticipants(context, params.meetingId);
...
export const POST = withEngineeringApiParams<{ meetingId: string }>(
  ...
  async (context, request, params) => {
    try {
      ...
      const data = await addParticipant(context, params.meetingId, body);
