...
import { getDocumentIntelligence } from "@/lib/project-intelligence/documents-service";
...
export const GET = withEngineeringApiParams(
  ...
  async (context, _request, { documentId }) => {
    try {
      ...
      return NextResponse.json({ data: await getDocumentIntelligence(context, documentId) });
    } catch (error) {
