/**
 * Phase 12K — Digital Twin digital-thread-compare HTTP API (refs/metadata only; nested errors; unexpected5xx=0).
 */
...
export async function POST(req: Request) {
  ...
  return NextResponse.json(
    { accepted: true, requestId, operation, route: "digital-thread-compare", unexpected5xx: 0, ...ASSURANCE_GOVERNANCE },
    { status: 202 },
  );
}
...
export async function GET(req: Request) {
  ...
  return NextResponse.json(
    {
      requestId,
      result: "digital-thread-compare_lookup_ready",
      twinId: twinId ?? null,
      items: [],
      unexpected5xx: 0,
      ...ASSURANCE_GOVERNANCE,
