/**
 * Minimum Asset Priority Context HTTP APIs.
 * Dimensional decision support — no opaque score, never a Health Index factor.
 */
...
export async function POST(req: Request) {
  ...
  const operation = typeof body.operation === "string" ? body.operation : "assess_priority";
  ...
  if (operation === "review" || operation === "publish") {
    const stateId =
      typeof body.priorityProfileId === "string" ? body.priorityProfileId : undefined;
    if (!stateId) {
      return err(400, "missing_state_id", "priorityProfileId is required", requestId);
    }
  }
...
export async function GET(req: Request) {
  ...
  const view = url.searchParams.get("view") ?? "priority";
  if (!tenantId || !workspaceId || !assetId) {
    return err(400, "missing_scope", "tenantId, workspaceId, and assetId are required", requestId);
  }
  return NextResponse.json({
    ...
    workspaceId,
    assetId,
    operation: view === "history" ? "read_priority_history" : "read_priority",
    result: "ok",
