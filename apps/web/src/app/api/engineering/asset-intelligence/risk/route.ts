/**
 * Minimum Asset Risk Signal HTTP APIs.
 * Advisory only — never creates or mutates canonical Engineering Core risk.
 */
...
export async function POST(req: Request) {
  ...
  const operation = typeof body.operation === "string" ? body.operation : "assess_risk";
  ...
  if (operation === "review" || operation === "publish") {
    const stateId =
      typeof body.riskSignalStateId === "string" ? body.riskSignalStateId : undefined;
    if (!stateId) {
      return err(400, "missing_state_id", "riskSignalStateId is required", requestId);
    }
  }
...
export async function GET(req: Request) {
  ...
  const view = url.searchParams.get("view") ?? "risk";
  if (!tenantId || !workspaceId || !assetId) {
    return err(400, "missing_scope", "tenantId, workspaceId, and assetId are required", requestId);
  }
  return NextResponse.json({
