...
export const POST = withBusinessApi(async ({ ctx }, request) => {
  ...
  if (!id || !rationale) {
    return NextResponse.json({ error: "id and rationale are required", code: "invalid_input" }, { status: 400 });
  }
  try {
    const data = await ctx.business.businessRisk.recordToleranceException(scope, id, rationale);
