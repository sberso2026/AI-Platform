...
import type { BusinessWorkCostIngestInput } from "@rtb/types";
...
export const GET = withBusinessApi(async ({ ctx }, request) => {
  ...
  try {
    const data = await ctx.business.workOperations.repository.listCostFacts(scope, workId);
...
export const POST = withBusinessApi(async ({ ctx }, request) => {
  ...
  try {
    const data = await ctx.business.workOperations.ingestCost(scope, body as unknown as BusinessWorkCostIngestInput);
