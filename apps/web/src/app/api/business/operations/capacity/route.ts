...
import type { BusinessWorkCapacityIngestInput } from "@rtb/types";
...
export const GET = withBusinessApi(async ({ ctx }, request) => {
  ...
  try {
    const data = await ctx.business.workOperations.repository.listCapacityFacts(scope, workId);
...
export const POST = withBusinessApi(async ({ ctx }, request) => {
  ...
  try {
    const data = await ctx.business.workOperations.ingestCapacity(
      scope,
      body as unknown as BusinessWorkCapacityIngestInput,
    );
