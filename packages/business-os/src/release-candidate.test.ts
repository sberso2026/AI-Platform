...
describe("BOS-13 release indicators", () => {
  it("declares RC with honest live/GA limitations and no new capability", () => {
    expect(BUSINESS_OS_VERSION).toBe("0.13.1");
    ...
    expect(BOS_PRODUCTION_GA_REMAINING_GATES).toContain("LIVE_RLS_NOT_CERTIFIED");
...
describe("BOS-13 connectors, secrets, SSRF, staging, CSV", () => {
  it("certifies contract/sandbox only and never infers live provider certification", async () => {
    ...
    expect(BOS_CONNECTOR_CERTIFICATION.xero.live).toBe("NOT_CERTIFIED");
    expect(BOS_CONNECTOR_CERTIFICATION.microsoft_365.live).toBe("NOT_CERTIFIED");
    expect(BOS_CONNECTOR_CERTIFICATION.hubspot.live).toBe("NOT_CERTIFIED");
