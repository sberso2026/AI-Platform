...
import {
  ...
  PHASE_11J_PROJECT_CONTROLS_SCENARIO_GATES,
  PHASE_11J_PROJECT_CONTROLS_SCENARIO_TABLES,
  PHASE_11J_PROJECT_CONTROLS_DECISION_TABLES,
...
const GATES_FILE = `${PC_CERT}/src/phase11i/gates.ts`;
const RUNNER_FILE = `${PC_CERT}/scripts/run-phase11j-certification.ts`;
const SECRET_SCAN_FILE = `${PC_CERT}/scripts/secret-exposure-scan.ts`;
const ARCH_TEST = "packages/platform-certification/src/phase11j-project-controls-risk-opportunity.test.ts";
const WORKFLOW = ".github/workflows/phase-11j-project-controls-risk-opportunity.yml";
...
const DOC_SCENARIO = "docs/architecture/PROJECT_CONTROLS_RISK_OPPORTUNITY_INTELLIGENCE.md";
const DOC_SCENARIO_MODEL = "docs/architecture/PROJECT_CONTROLS_RISK_OPPORTUNITY_MODEL.md";
...
const RISK_OPPORTUNITY_ROUTE = "apps/web/src/app/api/engineering/project-controls/risk-opportunity/route.ts";
...
const RISK_OPPORTUNITY_TEST = `${PC}/tests/phase11j-risk-opportunity-intelligence.test.ts`;

const SCENARIO_DOMAIN_FILES = [
  `${PC}/src/domain/risk-opportunity.ts`,
  `${PC}/src/domain/risk-opportunity-confidence.ts`,
  `${PC}/src/domain/risk-opportunity-engine.ts`,
  `${PC}/src/domain/project-context-composition.ts`,
  `${PC}/src/domain/engine-risk-opportunity.ts`,
] as const;
...
type GateResult = { id: Phase11jGateId; name: string; status: GateStatus; detail?: string };
    ...
    if (!createErr && created.user) {
      ...
      if (!signErr && signed.session?.access_token) {
        ...
        jwtMatrixOk =
          ...
          has(`${PC}/src/domain/role-matrix.ts`, /risk_opportunity\.assess/) &&
          has(`${PC}/src/domain/review-workflow.ts`, /risk_opportunity_self_approval_forbidden/);
      }
...
async function main() {
  ...
  const push = (id: Phase11jGateId, name: string, status: GateStatus, detail?: string) =>
  ...
  const SCENARIO = `${PC}/src/domain/risk-opportunity.ts`;
  const SCENARIO_ENGINE = `${PC}/src/domain/risk-opportunity-engine.ts`;
  const SCENARIO_CONFIDENCE = `${PC}/src/domain/risk-opportunity-confidence.ts`;
  const COMPOSITION = `${PC}/src/domain/project-context-composition.ts`;
  const ENGINE_RISK_OPPORTUNITY = `${PC}/src/domain/engine-risk-opportunity.ts`;
  ...
  const arch = run(
    "pnpm --filter @rtb/platform-certification exec -- vitest run src/phase11j-project-controls-risk-opportunity.test.ts src/phase11e-project-controls-cost.test.ts",
  );

  const forbidLocksOk = PHASE_11F_FORBIDDEN_CAPABILITIES.every((lock) =>
    has(VERSION, new RegExp(`${lock} = false`)),
  );
  const requiredFlagsOk = PHASE_11J_REQUIRED_CAPABILITIES.every((flag) =>
  ...
  push(
    ...
    exists("pnpm-workspace.yaml") &&
      ...
      exists(SPD_PKG) &&
      exists(PC_CERT_PKG) &&
      has(VERSION, /PROJECT_CONTROLS_VERSION = "0\.10\.0-risk-opportunity-intelligence"/) &&
      has(VERSION, /PROJECT_CONTROLS_STATUS = "risk_opportunity_intelligence"/) &&
      has(VERSION, /PROJECT_CONTROLS_PHASE = "11J"/) &&
      has(
        VERSION,
  ...
  push(
    ...
    has(
      ...
      has(OWNERSHIP_LOCK, /riskOpportunityIntelligenceOwnership/) &&
      ...
      has(
        VERSION,
        new RegExp(`riskOpportunityIntelligenceOwnership = "${PHASE_11J_SCENARIO_INTELLIGENCE_OWNER}"`),
      ) &&
      has(
        VERSION,
        new RegExp(
  ...
  push(
    "L",
    "Risk/opportunity terminology",
    exists(DOC_SCENARIO_MODEL) &&
      has(DOC_SCENARIO_MODEL, /RiskOpportunity Evidence/) &&
      has(DOC_SCENARIO_MODEL, /RiskOpportunity Control Context/) &&
      has(DOC_SCENARIO_MODEL, /RiskOpportunity Confidence/) &&
      has(SCENARIO, /export type RiskOpportunityEvidence/) &&
      has(SCENARIO, /export type RiskOpportunityControlContext/) &&
      has(SCENARIO, /export type RiskOpportunityAssessmentState/)
      ? "pass"
      : "fail",
  );
  push(
  ...
  push(
    "N",
    "Risk/opportunity Control Context",
    has(SCENARIO, /export type RiskOpportunityControlContext/) &&
      has(SCENARIO, /riskOpportunityUnitId/) &&
  ...
  push(
    "O",
    "Risk/opportunity Intelligence Engine",
    has(
      SCENARIO_ENGINE,
      /ProjectControlsRiskOpportunityIntelligenceEngine|RiskOpportunityIntelligenceEngine/,
    ) &&
      has(SCENARIO_ENGINE, /risk_opportunity_intelligence_advisory_v1/) &&
  ...
  push(
    "P",
    "Risk/opportunity Intelligence State",
    has(SCENARIO, /export type RiskOpportunityAssessmentState/) &&
      has(SCENARIO, /assessmentClass/) &&
      has(SCENARIO, /synthesis/) &&
      has(SCENARIO, /contributingContributors/) &&
      has(SCENARIO, /advisoryOnly: true/) &&
      has(SCENARIO, /duplicateRiskOwnershipDetected: false/)
      ? "pass"
      : "fail",
  );
  push(
    "Q",
    "Risk/opportunity taxonomy governance",
    has(SCENARIO, /RISK_SIGNALS/) &&
      has(SCENARIO, /OPPORTUNITY_SIGNALS/) &&
      has(SCENARIO, /emerging/) &&
      has(SCENARIO, /recovery/) &&
      has(SCENARIO, /isAbstainingRiskOpportunitySufficiency/) &&
      has(SCENARIO_ENGINE, /abstained/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(COMPOSITION, /ProjectContextCompositionEngine/) &&
      has(SCENARIO, /RISK_OPPORTUNITY_CONTRIBUTOR_KEYS/) &&
      has(SCENARIO, /export type RiskOpportunityContributorRef/) &&
  ...
  push(
    ...
    has(SCENARIO, /export type RiskOpportunityEvidence/) &&
      ...
      has(SCENARIO, /riskRegisterMutationClaimed: false/) &&
      has(SCENARIO, /ownerAssignmentClaimed: false/) &&
  ...
  push(
    ...
    has(SCENARIO, /"sufficient"/) &&
      ...
      has(SCENARIO, /"stale"/) &&
      has(SCENARIO_CONFIDENCE, /createRiskOpportunityConfidenceEngine|RiskOpportunityConfidenceEngine/) &&
      has(SCENARIO, /autoExecutionClaimed: false/) &&
      has(SCENARIO, /approvalAuthorityClaimed: false/) &&
      has(VERSION, /RISK_OPPORTUNITY_CONFIDENCE_ENGINE_READY = true/)
      ? "pass"
      : "fail",
  );
  push(
    "U",
    "Risk/opportunity unit governance",
    has(SCENARIO, /riskOpportunityUnitId/) &&
      has(SCENARIO_ENGINE, /risk_opportunity_unit_id_required/) &&
      has(BATCH_70, /risk_opportunity_unit_id/) &&
      has(RISK_OPPORTUNITY_ROUTE, /missing_risk_opportunity_unit/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(SCENARIO_ENGINE, /asOf/) &&
      ...
      has(BATCH_70, /assessed_at/) &&
      has(BATCH_70, /recorded_at/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    "Contributor refs",
    has(SCENARIO, /RISK_OPPORTUNITY_CONTRIBUTOR_KEYS/) &&
      has(SCENARIO, /RiskOpportunityContributorRef/) &&
  ...
  push(
    ...
    has(CONTEXT, /ProjectContextEngine/) &&
      has(CONTEXT, /key: "risk_opportunity_intelligence"/) &&
      has(CONTEXT, /status: "active"/) &&
      has(CONTEXT, /RiskOpportunityAssessmentState/) &&
  ...
  push(
    ...
    exists(PROFILE_ROUTE) &&
      has(PROFILE_ROUTE, /riskOpportunityIntelligenceReady:\s*true/) &&
      has(PROFILE_ROUTE, /scenarioIntelligenceReady:\s*true/) &&
      has(PROFILE_ROUTE, /"risk_opportunity_intelligence"/) &&
      has(PROFILE_ROUTE, /projectContextCompositionReady:\s*true/) &&
      has(BATCH_70, /risk_opportunity_summary/) &&
      has(POSTGRES, /risk_opportunity_summary/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(BATCH_70, /risk_opportunity_state_ids/) &&
      has(POSTGRES, /risk_opportunity_state_ids/) &&
      has(`${PC}/src/domain/change.ts`, /riskOpportunityStateIds/) &&
  ...
  push(
    "AB",
    "Governed review",
    has(REVIEW, /project_controls\.risk_opportunity_review/) &&
      has(REVIEW, /RISK_OPPORTUNITY_REVIEW_WORKFLOW/) &&
      has(REVIEW, /EngineeringWorkflowDefinition/) &&
      has(VERSION, /RISK_OPPORTUNITY_REVIEW_WORKFLOW_READY = true/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(REVIEW, /risk_opportunity_self_approval_forbidden/) &&
      has(REVIEW, /risk_opportunity_publish_requires_approved_review/) &&
      has(REVIEW, /risk_opportunity_assessment_approval_is_not_register_approval/) &&
      has(VERSION, /AI_MAY_PUBLISH_RISK_OPPORTUNITY_FORBIDDEN = true/) &&
      has(VERSION, /AUTONOMOUS_RISK_OPPORTUNITY_PUBLICATION_ALLOWED = false/) &&
      ...
      has(RISK_OPPORTUNITY_ROUTE, /self_approval_forbidden/)
      ? "pass"
      : "fail",
  );
  ...
  push(
    ...
    "Hosted migration",
    exists(BATCH_70) &&
      PHASE_11J_PROJECT_CONTROLS_SCENARIO_TABLES.every((table) =>
        has(BATCH_70, new RegExp(`CREATE TABLE IF NOT EXISTS ${table}`)),
      ) &&
      has(BATCH_70, /ENABLE ROW LEVEL SECURITY/) &&
      has(BATCH_70, /CREATE POLICY/) &&
      exists(BATCH_62) &&
      exists(BATCH_63) &&
      exists(BATCH_64) &&
      exists(BATCH_65) &&
  ...
  push(
    "AE",
    "Hosted persistence",
    PHASE_11J_PROJECT_CONTROLS_SCENARIO_TABLES.every((table) => has(POSTGRES, new RegExp(table))) &&
      has(PERSISTENCE, /riskOpportunityStates/) &&
      has(PERSISTENCE, /riskOpportunityEvidence/) &&
      has(PERSISTENCE, /riskOpportunityReviews/) &&
      has(PERSISTENCE, /riskOpportunityConfidence/) &&
      has(`${PC}/src/domain/repository-factory.ts`, /createProjectControlsRepository/) &&
      has(VERSION, /RISK_OPPORTUNITY_PERSISTENCE_READY = true/) &&
      hostedAvailable &&
      hosted.tablesOk
      ? "pass"
      : "fail",
  ...
  push(
    ...
    has(VERSION, /RESOURCE_PLANNING_IMPLEMENTED = false/) &&
      ...
      has(SCENARIO_ENGINE, /assertNoPredictiveScheduling/)
      ? "pass"
      : "fail",
  );
  ...
  push(
    ...
    has(SCENARIO_ENGINE, /assertNoEarnedValueOrCpm/) &&
      ...
      has(BATCH_70, /pc_ro_no_earned_value/) &&
  ...
  push(
    ...
    has(SCENARIO_ENGINE, /assertNoEarnedValueOrCpm/) &&
      ...
      has(VERSION, /DECISION_ENGINE_IMPLEMENTED = false/) &&
      has(BATCH_70, /pc_ro_no_predictive_scheduling/) &&
  ...
  push(
    ...
    has(SCENARIO, /numericalPrecisionClaimed: false/) &&
      has(SCENARIO_ENGINE, /predictiveSchedulingPerformed: false/) &&
      has(BATCH_70, /pc_ro_no_auto_execution/) &&
      has(RISK_OPPORTUNITY_ROUTE, /completionDateClaimed/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(SCENARIO, /monteCarloPerformed: false/) &&
      has(VERSION, /DECISIONING_IMPLEMENTED = false/) &&
      has(BATCH_70, /pc_ro_no_contract_instruction/) &&
      has(RISK_OPPORTUNITY_ROUTE, /costDecisionClaimed/)
      ? "pass"
      : "fail",
  );

  // ------------------------------------------------------------------ AN–AW
  const eventsOk = PHASE_11J_SCENARIO_EVENTS.every((event) =>
  ...
  push(
    ...
    eventsOk &&
      PHASE_11J_SCENARIO_EVENTS.every((event) =>
        has(BATCH_70, new RegExp(event.replace(/\./g, "\\."))),
      ) &&
      has(POSTGRES, /project_controls_outbox_events/) &&
      has(EVENTS, /riskOpportunityEventPayload/) &&
  ...
  push(
    "AO",
    "HTTP contracts",
    exists(RISK_OPPORTUNITY_ROUTE) &&
      has(RISK_OPPORTUNITY_ROUTE, /riskOpportunityIntelligenceReady:\s*true/) &&
      has(RISK_OPPORTUNITY_ROUTE, /projectContextCompositionReady:\s*true/) &&
      has(RISK_OPPORTUNITY_ROUTE, /resourcePlanningImplemented:\s*false/) &&
      has(RISK_OPPORTUNITY_ROUTE, /decisionExecutionImplemented:\s*false/) &&
      has(RISK_OPPORTUNITY_ROUTE, /automaticRiskRegisterMutationEnabled:\s*false/) &&
      has(RISK_OPPORTUNITY_ROUTE, /automaticTreatmentExecutionEnabled:\s*false/) &&
      has(RISK_OPPORTUNITY_ROUTE, /predictiveSchedulingImplemented:\s*false/) &&
      has(RISK_OPPORTUNITY_ROUTE, /phase11kReady:\s*true/) &&
      has(RISK_OPPORTUNITY_ROUTE, /assess_risk_opportunity/) &&
      has(RISK_OPPORTUNITY_ROUTE, /error:\s*\{\s*code/) &&
      has(RISK_OPPORTUNITY_ROUTE, /requestId/)
      ? "pass"
      : "fail",
  );
  push(
    "AP",
    "JWT role matrix",
    has(ROLES, /"risk_opportunity\.read"/) &&
      has(ROLES, /"risk_opportunity\.assess"/) &&
      has(ROLES, /"risk_opportunity\.review"/) &&
      has(ROLES, /"risk_opportunity\.publish"/) &&
  ...
  push(
    "AQ",
    "Tenant isolation",
    has(BATCH_70, /tenant_id/) &&
      has(BATCH_70, /get_user_tenant_ids/) &&
  ...
  push(
    "AR",
    "Workspace isolation",
    has(BATCH_70, /workspace_id/) &&
      has(BATCH_70, /workspace_memberships/) &&
  ...
  push(
    ...
    has(SCENARIO_ENGINE, /scope_project_mismatch/) &&
      has(RISK_OPPORTUNITY_ROUTE, /missing_scope/) &&
      has(SCENARIO_ENGINE, /project_id_required/) &&
      has(`${PC}/src/domain/engine.ts`, /assessRiskOpportunity/)
      ? "pass"
      : "fail",
  );
  push(
  ...
  push(
    ...
    has(PERSISTENCE, /optimistic_lock_conflict/) &&
      ...
      has(BATCH_70, /UNIQUE \(tenant_id, workspace_id, project_id/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    "Observability",
    has(RISK_OPPORTUNITY_ROUTE, /requestId/) &&
      has(RISK_OPPORTUNITY_ROUTE, /correlationId/) &&
      has(RISK_OPPORTUNITY_ROUTE, /durationMs/) &&
      has(RISK_OPPORTUNITY_ROUTE, /x-request-id/) &&
  ...
  push(
    ...
    has(SCENARIO_ROUTE, /durationMs/) &&
      ...
      test11j.ok && test11i.ok && test11f.ok
      ? "pass"
      : "fail",
    `${test11j.detail ?? ""};${test11i.detail ?? ""};${test11f.detail ?? ""}`,
  );
  ...
  push(
    ...
    has(VERSION, /CPM_SCHEDULING_IMPLEMENTED = false/) &&
      ...
      has(BATCH_70, /pc_ro_no_cpm/) &&
      has(BATCH_70, /pc_ro_no_float/) &&
  ...
  push(
    ...
    has(VERSION, /SCHEDULE_EXECUTION_IMPLEMENTED = false/) &&
      has(BATCH_70, /pc_ro_no_schedule_execution/) &&
  ...
  const RiskOpportunityIntelligenceReady =
    has(VERSION, /RISK_OPPORTUNITY_INTELLIGENCE_READY = true/) &&
    has(CONTEXT, /key: "risk_opportunity_intelligence"/) &&
    has(COMPOSITION, /ProjectContextCompositionEngine/) &&
    has(SCENARIO_ENGINE, /RiskOpportunityIntelligenceEngine/);
  const releaseEligible =
    RiskOpportunityIntelligenceReady &&
    has(VERSION, /SCENARIO_INTELLIGENCE_READY = true/) &&
    aiSurfaceUnchanged &&
    piSurfaceUnchanged &&
    iiSurfaceUnchanged &&
    batch61To66Unchanged &&
    has(PC_PKG, /"version": "0\.10\.0-risk-opportunity-intelligence"/) &&
    has(PC_CERT_PKG, /"version": "0\.10\.0-risk-opportunity-intelligence"/) &&
    has(VERSION, /PROGRESS_INTELLIGENCE_11B_INTACT = true/) &&
    has(VERSION, /CHANGE_INTELLIGENCE_11D_INTACT = true/) &&
    has(VERSION, /SCHEDULE_INTELLIGENCE_11C_INTACT = true/) &&
    has(VERSION, /PRODUCTIVITY_INTELLIGENCE_11F_INTACT = true/) &&
  ...
  const phase11kReady = has(VERSION, /PHASE_11K_READY = true/);

  push(
    "BC",
    "Phase 11K readiness (flag only)",
    phase11kReady &&
      has(VERSION, /RISK_OPPORTUNITY_INTELLIGENCE_READY = true/) &&
      has(VERSION, /SCENARIO_INTELLIGENCE_READY = true/)
      ? "pass"
      : "fail",
    `phase11kReady=${phase11kReady};riskOpportunityReady=${has(VERSION, /RISK_OPPORTUNITY_INTELLIGENCE_READY = true/)};scenarioReady=${has(VERSION, /SCENARIO_INTELLIGENCE_READY = true/)};hosted=${hostedAvailable}`,
  );
  ...
  const artifact = {
    schemaVersion: "phase11j-project-controls-risk-opportunity/1",
    phase: "11J",
    title: "Project Controls Risk & Opportunity Intelligence",
    moduleKey: "project_controls",
    version: PHASE_11J_PROJECT_CONTROLS_VERSION,
    status: "risk_opportunity_intelligence",
    ...
    riskOpportunityIntelligenceOwnership: PHASE_11J_SCENARIO_INTELLIGENCE_OWNER,
    ...
    scenarioIntelligenceOwnership: PHASE_11J_SCENARIO_INTELLIGENCE_OWNER,
    riskOpportunityTables: [...PHASE_11J_PROJECT_CONTROLS_SCENARIO_TABLES],
    decisionTables: [...PHASE_11J_PROJECT_CONTROLS_DECISION_TABLES],
    ...
    riskOpportunityEvents: [...PHASE_11J_SCENARIO_EVENTS],
    phase11hReady: true,
    phase11iReady: true,
    phase11jReady: true,
    phase11kReady: phase11kReady,
    releaseEligible: pass && releaseEligible,
    requiredTestsSkipped: false,
    unexpected5xx: false,
    gates,
    requiredGates: PHASE_11J_PROJECT_CONTROLS_SCENARIO_GATES.map(([id]) => id),
    gateCount: PHASE_11J_GATE_COUNT,
  ...
  const outPath = resolve(outDir, "phase11j-project-controls-risk-opportunity-certification.json");
