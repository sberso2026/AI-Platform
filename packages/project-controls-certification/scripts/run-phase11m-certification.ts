/**
 * Phase 11M certification runner (gates A–BC) — Project Controls Organizational Learning Intelligence while Progress (11B) through Explainability (11L) stay intact.
 */
...
import {
  ...
  PHASE_11M_PROJECT_CONTROLS_SCENARIO_GATES,
  PHASE_11M_PROJECT_CONTROLS_SCENARIO_TABLES,
  PHASE_11M_PROJECT_CONTROLS_DECISION_TABLES,
...
const GATES_FILE = `${PC_CERT}/src/phase11m/gates.ts`;
const RUNNER_FILE = `${PC_CERT}/scripts/run-phase11m-certification.ts`;
const SECRET_SCAN_FILE = `${PC_CERT}/scripts/secret-exposure-scan.ts`;
const ARCH_TEST =
  "packages/platform-certification/src/phase11m-project-controls-organizational-learning.test.ts";
const WORKFLOW = ".github/workflows/phase-11m-project-controls-organizational-learning.yml";
...
const DOC_SCENARIO = "docs/architecture/PROJECT_CONTROLS_ORGANIZATIONAL_LEARNING_INTELLIGENCE.md";
const DOC_SCENARIO_MODEL = "docs/architecture/PROJECT_CONTROLS_ORGANIZATIONAL_LEARNING_MODEL.md";
...
const ORGANIZATIONAL_LEARNING_ROUTE = "apps/web/src/app/api/engineering/project-controls/organizational-learning/route.ts";
...
const ORGANIZATIONAL_LEARNING_TEST = `${PC}/tests/phase11m-organizational-learning.test.ts`;

const ORGANIZATIONAL_LEARNING_DOMAIN_FILES = [
  `${PC}/src/domain/organizational-learning.ts`,
  `${PC}/src/domain/organizational-learning-confidence.ts`,
  `${PC}/src/domain/organizational-learning-engine.ts`,
  `${PC}/src/domain/project-context-composition.ts`,
  `${PC}/src/domain/engine-organizational-learning.ts`,
] as const;
...
const DOMAIN_FILES = [
  ...
  ...CHANGE_DOMAIN_FILES,
  ...COST_DOMAIN_FILES,
  ...ORGANIZATIONAL_LEARNING_DOMAIN_FILES,
...
type GateResult = { id: Phase11mGateId; name: string; status: GateStatus; detail?: string };
    ...
    if (!createErr && created.user) {
      ...
      if (!signErr && signed.session?.access_token) {
        ...
        jwtMatrixOk =
          ...
          has(`${PC}/src/domain/role-matrix.ts`, /organizational_learning\.assess/) &&
          has(`${PC}/src/domain/review-workflow.ts`, /organizational_learning_self_approval_forbidden/);
      }
...
async function main() {
  ...
  const push = (id: Phase11mGateId, name: string, status: GateStatus, detail?: string) =>
  ...
  const SCENARIO = `${PC}/src/domain/organizational-learning.ts`;
  const SCENARIO_ENGINE = `${PC}/src/domain/organizational-learning-engine.ts`;
  const SCENARIO_CONFIDENCE = `${PC}/src/domain/organizational-learning-confidence.ts`;
  const COMPOSITION = `${PC}/src/domain/project-context-composition.ts`;
  const ENGINE_ASSURANCE = `${PC}/src/domain/engine-organizational-learning.ts`;
  ...
  const test11m = run(
    `pnpm --filter @rtb/project-controls exec -- vitest run tests/phase11m-organizational-learning.test.ts`,
  );
  const test11f = run(
  ...
  const arch = run(
    "pnpm --filter @rtb/platform-certification exec -- vitest run src/phase11m-project-controls-organizational-learning.test.ts src/phase11e-project-controls-cost.test.ts",
  );

  const forbidLocksOk = PHASE_11F_FORBIDDEN_CAPABILITIES.every((lock) =>
    has(VERSION, new RegExp(`${lock} = false`)),
  );
  const requiredFlagsOk = PHASE_11M_REQUIRED_CAPABILITIES.every((flag) =>
  ...
  push(
    ...
    exists("pnpm-workspace.yaml") &&
      ...
      exists(SPD_PKG) &&
      exists(PC_CERT_PKG) &&
      has(VERSION, /PROJECT_CONTROLS_VERSION = "0.13.0-organizational-learning"/) &&
      has(VERSION, /PROJECT_CONTROLS_STATUS = "organizational_learning"/) &&
      has(VERSION, /PROJECT_CONTROLS_PHASE = "11M"/) &&
      has(
        VERSION,
      ...
      has(VERSION, /PHASE_11L_CERTIFIED_COMMIT =\s*\r?\n?\s*"5176bed8168ad39cca4de43b2f95737aab6569aa"/) &&
      has(VERSION, /PHASE_11L_HOSTED_RUN = "31249492990"/)
      ? "pass"
      : "fail",
    buildIdentitySha,
  );
  ...
  push(
    ...
    has(
      ...
      has(OWNERSHIP_LOCK, /organizationalLearningIntelligenceOwnership/) &&
      has(OWNERSHIP_LOCK, /organizational_learning_controls_intelligence/) &&
      ...
      has(
        VERSION,
        new RegExp(`organizationalLearningIntelligenceOwnership = "${PHASE_11M_SCENARIO_INTELLIGENCE_OWNER}"`),
      ) &&
      has(
        VERSION,
        new RegExp(
  ...
  push(
    "L",
    "Organizational learning terminology",
    exists(DOC_SCENARIO_MODEL) &&
      has(DOC_SCENARIO_MODEL, /OrganizationalLearningAssessmentState/) &&
      has(DOC_SCENARIO_MODEL, /Organizational learning control context|OrganizationalLearningControlContext/) &&
      has(DOC_SCENARIO_MODEL, /Organizational learning confidence|OrganizationalLearningConfidence/) &&
      has(SCENARIO, /export type OrganizationalLearningEvidence/) &&
      has(SCENARIO, /export type OrganizationalLearningControlContext/) &&
      has(SCENARIO, /export type OrganizationalLearningAssessmentState/)
      ? "pass"
      : "fail",
  );
  push(
  ...
  push(
    "N",
    "Organizational learning control context",
    has(SCENARIO, /export type OrganizationalLearningControlContext/) &&
      has(SCENARIO, /organizationalLearningUnitId/) &&
  ...
  push(
    "O",
    "Organizational learning intelligence engine",
    has(SCENARIO_ENGINE, /ProjectControlsOrganizationalLearningEngine/) &&
      has(SCENARIO_ENGINE, /organizational_learning_intelligence_advisory_v1/) &&
      has(SCENARIO_ENGINE, /abstained/) &&
      has(VERSION, /ORGANIZATIONAL_LEARNING_INTELLIGENCE_READY = true/)
      ? "pass"
      : "fail",
  );
  push(
    "P",
    "Organizational learning intelligence state",
    has(SCENARIO, /export type OrganizationalLearningAssessmentState/) &&
      has(SCENARIO, /assessmentClass/) &&
      has(SCENARIO, /synthesis/) &&
      has(SCENARIO, /contributingContributors/) &&
      has(SCENARIO, /advisoryOnly: true/) &&
      has(SCENARIO, /duplicateKnowledgeOwnershipDetected: false/)
      ? "pass"
      : "fail",
  );
  push(
    "Q",
    "Organizational learning taxonomy governance",
    has(SCENARIO, /LEARNING_TAXONOMY/) &&
      has(SCENARIO, /LEARNING_BASIS_REASONS/) &&
      has(SCENARIO, /unknown/) &&
      has(SCENARIO, /insufficient_evidence/) &&
      has(SCENARIO, /isAbstainingOrganizationalLearningSufficiency/) &&
      has(SCENARIO_ENGINE, /abstained/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(COMPOSITION, /ProjectContextCompositionEngine/) &&
      has(SCENARIO, /ORGANIZATIONAL_LEARNING_CONTRIBUTOR_KEYS/) &&
      has(SCENARIO, /export type OrganizationalLearningContributorRef/) &&
  ...
  push(
    "S",
    "Organizational learning evidence",
    has(SCENARIO, /export type OrganizationalLearningEvidence/) &&
      has(SCENARIO, /sourceType/) &&
      has(SCENARIO, /sourceRef/) &&
      has(SCENARIO, /provenance/) &&
      has(SCENARIO, /autoExecutionClaimed: false/) &&
      has(SCENARIO, /learningApprovalClaimed: false/) &&
      has(SCENARIO, /learningApprovalClaimed: false/) &&
      has(SCENARIO, /knowledgeMutationClaimed: false/) &&
      has(SCENARIO, /registerMutationClaimed: false/) &&
      has(SCENARIO, /recommendationClaimed: false/) &&
  ...
  push(
    "T",
    "Organizational learning confidence",
    has(SCENARIO, /"sufficient"/) &&
      has(SCENARIO, /"limited"/) &&
      has(SCENARIO, /"insufficient"/) &&
      has(SCENARIO, /"conflicting"/) &&
      has(SCENARIO, /"incomplete"/) &&
      has(SCENARIO_CONFIDENCE, /createOrganizationalLearningConfidenceEngine|OrganizationalLearningConfidenceEngine/) &&
      has(SCENARIO, /knowledgeMutationClaimed: false/) &&
      has(SCENARIO, /learningApprovalClaimed: false/) &&
      has(VERSION, /ORGANIZATIONAL_LEARNING_CONFIDENCE_ENGINE_READY = true/)
      ? "pass"
      : "fail",
  );
  push(
    "U",
    "Organizational learning unit governance",
    has(SCENARIO, /organizationalLearningUnitId/) &&
      has(SCENARIO_ENGINE, /organizational_learning_unit_id_required/) &&
      has(BATCH_73, /organizational_learning_unit_id/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /missing_organizational_learning_unit/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(SCENARIO_ENGINE, /asOf/) &&
      ...
      has(BATCH_73, /assessed_at/) &&
      has(BATCH_73, /recorded_at/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    "Contributor refs",
    has(SCENARIO, /ORGANIZATIONAL_LEARNING_CONTRIBUTOR_KEYS/) &&
      has(SCENARIO, /OrganizationalLearningContributorRef/) &&
  ...
  push(
    ...
    has(CONTEXT, /ProjectContextEngine/) &&
      has(CONTEXT, /key: "organizational_learning"/) &&
      has(CONTEXT, /status: "active"/) &&
      has(CONTEXT, /OrganizationalLearningAssessmentState/) &&
  ...
  push(
    ...
    exists(PROFILE_ROUTE) &&
      ...
      has(PROFILE_ROUTE, /organizationalLearningReady:\s*true/) &&
      has(PROFILE_ROUTE, /organizationalLearningIntelligenceReady:\s*true/) &&
      has(PROFILE_ROUTE, /assuranceIntelligenceReady:\s*true/) &&
      has(PROFILE_ROUTE, /scenarioIntelligenceReady:\s*true/) &&
      has(PROFILE_ROUTE, /"organizational_learning"/) &&
      has(PROFILE_ROUTE, /phase11nReady:\s*true/) &&
      has(PROFILE_ROUTE, /projectContextCompositionReady:\s*true/) &&
      has(BATCH_73, /organizational_learning_summary/) &&
      has(POSTGRES, /organizational_learning_summary/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(BATCH_73, /organizational_learning_state_ids/) &&
      has(POSTGRES, /organizational_learning_state_ids/) &&
      has(`${PC}/src/domain/change.ts`, /organizationalLearningStateIds/) &&
  ...
  push(
    "AB",
    "Governed review",
    has(REVIEW, /project_controls\.organizational_learning_review/) &&
      has(REVIEW, /ORGANIZATIONAL_LEARNING_REVIEW_WORKFLOW/) &&
      has(REVIEW, /EngineeringWorkflowDefinition/) &&
      has(VERSION, /ORGANIZATIONAL_LEARNING_REVIEW_WORKFLOW_READY = true/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(REVIEW, /organizational_learning_self_approval_forbidden/) &&
      has(REVIEW, /organizational_learning_publish_requires_approved_review/) &&
      has(REVIEW, /organizational_learning_assessment_is_not_approval_or_fabricated_lesson/) &&
      has(VERSION, /AI_MAY_PUBLISH_ORGANIZATIONAL_LEARNING_FORBIDDEN = true/) &&
      has(VERSION, /AUTONOMOUS_ORGANIZATIONAL_LEARNING_PUBLICATION_ALLOWED = false/) &&
      ...
      has(ORGANIZATIONAL_LEARNING_ROUTE, /self_approval_forbidden/)
      ? "pass"
      : "fail",
  );
  ...
  push(
    ...
    "Hosted migration",
    exists(BATCH_73) &&
      PHASE_11M_PROJECT_CONTROLS_SCENARIO_TABLES.every((table) =>
        has(BATCH_73, new RegExp(`CREATE TABLE IF NOT EXISTS ${table}`)),
      ) &&
      has(BATCH_73, /ENABLE ROW LEVEL SECURITY/) &&
      has(BATCH_73, /CREATE POLICY/) &&
      exists(BATCH_62) &&
      exists(BATCH_63) &&
      exists(BATCH_64) &&
      exists(BATCH_65) &&
  ...
  push(
    "AE",
    "Hosted persistence",
    PHASE_11M_PROJECT_CONTROLS_SCENARIO_TABLES.every((table) => has(POSTGRES, new RegExp(table))) &&
      has(PERSISTENCE, /organizationalLearningStates/) &&
      has(PERSISTENCE, /organizationalLearningEvidence/) &&
      has(PERSISTENCE, /organizationalLearningReviews/) &&
      has(PERSISTENCE, /organizationalLearningConfidence/) &&
      has(`${PC}/src/domain/repository-factory.ts`, /createProjectControlsRepository/) &&
      has(VERSION, /ORGANIZATIONAL_LEARNING_PERSISTENCE_READY = true/) &&
      hostedAvailable &&
      hosted.tablesOk
      ? "pass"
      : "fail",
  ...
  push(
    ...
    has(SCENARIO_ENGINE, /assertNoEarnedValueOrCpm/) &&
      ...
      has(BATCH_73, /pc_as_no_earned_value/) &&
  ...
  push(
    ...
    has(SCENARIO_ENGINE, /assertNoEarnedValueOrCpm/) &&
      ...
      has(VERSION, /DECISION_ENGINE_IMPLEMENTED = false/) &&
      has(BATCH_73, /pc_as_no_predictive_scheduling/) &&
  ...
  push(
    ...
    has(SCENARIO, /predictiveSchedulingPerformed: false/) &&
      has(SCENARIO_ENGINE, /predictiveSchedulingPerformed: false/) &&
      has(BATCH_73, /pc_as_no_auto_execution/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /completionDateClaimed/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(SCENARIO, /predictiveSchedulingPerformed: false/) &&
      has(VERSION, /DECISIONING_IMPLEMENTED = false/) &&
      has(BATCH_73, /pc_as_no_contract_instruction/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /monteCarloClaimed/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /costDecisionClaimed/)
      ? "pass"
      : "fail",
  );

  // ------------------------------------------------------------------ AN–AW
  const eventsOk = PHASE_11M_SCENARIO_EVENTS.every((event) =>
  ...
  push(
    ...
    eventsOk &&
      PHASE_11M_SCENARIO_EVENTS.every((event) =>
        has(BATCH_73, new RegExp(event.replace(/\./g, "\\."))),
      ) &&
      has(POSTGRES, /project_controls_outbox_events/) &&
      has(EVENTS, /organizationalLearningEventPayload/) &&
  ...
  push(
    "AO",
    "HTTP contracts",
    exists(ORGANIZATIONAL_LEARNING_ROUTE) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /organizationalLearningIntelligenceReady:\s*true/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /projectContextCompositionReady:\s*true/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /resourcePlanningImplemented:\s*false/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /decisionExecutionImplemented:\s*false/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /automaticLearningApprovalEnabled:\s*false/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /automaticKnowledgeMutationEnabled:\s*false/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /predictiveSchedulingImplemented:\s*false/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /phase11nReady:\s*true/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /assess_organizational_learning/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /error:\s*\{\s*code/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /requestId/)
      ? "pass"
      : "fail",
  );
  push(
    "AP",
    "JWT role matrix",
    has(ROLES, /"organizational_learning\.read"/) &&
      has(ROLES, /"organizational_learning\.assess"/) &&
      has(ROLES, /"organizational_learning\.review"/) &&
      has(ROLES, /"organizational_learning\.publish"/) &&
  ...
  push(
    "AQ",
    "Tenant isolation",
    has(BATCH_73, /tenant_id/) &&
      has(BATCH_73, /get_user_tenant_ids/) &&
  ...
  push(
    "AR",
    "Workspace isolation",
    has(BATCH_73, /workspace_id/) &&
      has(BATCH_73, /workspace_memberships/) &&
  ...
  push(
    ...
    has(SCENARIO_ENGINE, /scope_project_mismatch/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /missing_scope/) &&
      has(SCENARIO_ENGINE, /project_id_required/) &&
      has(`${PC}/src/domain/engine.ts`, /assessOrganizationalLearning/)
      ? "pass"
      : "fail",
  );
  push(
  ...
  push(
    ...
    has(PERSISTENCE, /optimistic_lock_conflict/) &&
      ...
      has(BATCH_73, /UNIQUE \(tenant_id, workspace_id, project_id/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    "Observability",
    has(ORGANIZATIONAL_LEARNING_ROUTE, /requestId/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /correlationId/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /durationMs/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /x-request-id/) &&
  ...
  push(
    ...
    "Performance",
    has(ORGANIZATIONAL_LEARNING_ROUTE, /durationMs/) &&
      has(ORGANIZATIONAL_LEARNING_ROUTE, /started = Date\.now/) &&
      test11m.ok && test11j.ok && test11i.ok && test11f.ok
      ? "pass"
      : "fail",
    `${test11m.detail ?? ""};${test11j.detail ?? ""};${test11i.detail ?? ""};${test11f.detail ?? ""}`,
  );
  ...
  push(
    ...
    has(VERSION, /CPM_SCHEDULING_IMPLEMENTED = false/) &&
      ...
      has(BATCH_73, /pc_as_no_cpm/) &&
      has(BATCH_73, /pc_as_no_float/) &&
  ...
  push(
    ...
    has(VERSION, /SCHEDULE_EXECUTION_IMPLEMENTED = false/) &&
      has(BATCH_73, /pc_as_no_schedule_execution/) &&
  ...
  const organizationalLearningIntelligenceReady =
    has(VERSION, /ORGANIZATIONAL_LEARNING_INTELLIGENCE_READY = true/) &&
    has(CONTEXT, /key: "organizational_learning"/) &&
    has(COMPOSITION, /ProjectContextCompositionEngine/) &&
    has(SCENARIO_ENGINE, /ProjectControlsOrganizationalLearningEngine/);
  const releaseEligible =
    organizationalLearningIntelligenceReady &&
    has(VERSION, /SCENARIO_INTELLIGENCE_READY = true/) &&
    aiSurfaceUnchanged &&
    piSurfaceUnchanged &&
    iiSurfaceUnchanged &&
    batch61To66Unchanged &&
    has(PC_PKG, /"version": "0.13.0-organizational-learning"/) &&
    has(PC_CERT_PKG, /"version": "0.13.0-organizational-learning"/) &&
    has(VERSION, /PROGRESS_INTELLIGENCE_11B_INTACT = true/) &&
    has(VERSION, /CHANGE_INTELLIGENCE_11D_INTACT = true/) &&
    has(VERSION, /SCHEDULE_INTELLIGENCE_11C_INTACT = true/) &&
    has(VERSION, /PRODUCTIVITY_INTELLIGENCE_11F_INTACT = true/) &&
  ...
  const phase11nReady = has(VERSION, /PHASE_11N_READY = true/);

  push(
    "BC",
    "Phase 11N readiness (flag only)",
    phase11nReady &&
      has(VERSION, /ORGANIZATIONAL_LEARNING_INTELLIGENCE_READY = true/) &&
      ...
      ? "pass"
      : "fail",
    `phase11nReady=${phase11nReady};organizationalLearningReady=${has(VERSION, /ORGANIZATIONAL_LEARNING_INTELLIGENCE_READY = true/)};explainabilityReady=${has(VERSION, /EXPLAINABILITY_INTELLIGENCE_READY = true/)};assuranceReady=${has(VERSION, /ASSURANCE_INTELLIGENCE_READY = true/)};hosted=${hostedAvailable}`,
  );
  ...
  const artifact = {
    schemaVersion: "phase11m-project-controls-organizational-learning/1",
    phase: "11M",
    title: "Project Controls Organizational Learning Intelligence",
    moduleKey: "project_controls",
    version: PHASE_11M_PROJECT_CONTROLS_VERSION,
    status: "organizational_learning",
    ...
    organizationalLearningIntelligenceOwnership: PHASE_11M_SCENARIO_INTELLIGENCE_OWNER,
    ...
    scenarioIntelligenceOwnership: PHASE_11M_SCENARIO_INTELLIGENCE_OWNER,
    explainabilityTables: [
    ...
    organizationalLearningTables: [...PHASE_11M_PROJECT_CONTROLS_SCENARIO_TABLES],
    decisionTables: [...PHASE_11M_PROJECT_CONTROLS_DECISION_TABLES],
    ...
    organizationalLearningEvents: [...PHASE_11M_SCENARIO_EVENTS],
    ...
    RiskOpportunityIntelligenceReady: true,
    phase11nReady: phase11nReady,
    releaseEligible: pass && releaseEligible,
    requiredTestsSkipped: false,
    unexpected5xx: false,
    gates,
    requiredGates: PHASE_11M_PROJECT_CONTROLS_SCENARIO_GATES.map(([id]) => id),
    gateCount: PHASE_11M_GATE_COUNT,
  ...
  const outPath = resolve(outDir, "phase11m-project-controls-organizational-learning-certification.json");
