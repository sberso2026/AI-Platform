...
import {
  ...
  PHASE_11G_PROJECT_CONTROLS_FORECAST_GATES,
  PHASE_11G_PROJECT_CONTROLS_FORECAST_TABLES,
...
const GATES_FILE = `${PC_CERT}/src/phase11g/gates.ts`;
const RUNNER_FILE = `${PC_CERT}/scripts/run-phase11g-certification.ts`;
const SECRET_SCAN_FILE = `${PC_CERT}/scripts/secret-exposure-scan.ts`;
const ARCH_TEST = "packages/platform-certification/src/phase11g-project-controls-forecast.test.ts";
const WORKFLOW = ".github/workflows/phase-11g-project-controls-forecast.yml";
...
const DOC_PRODUCTIVITY = "docs/architecture/PROJECT_CONTROLS_FORECAST_INTELLIGENCE.md";
const DOC_PRODUCTIVITY_MODEL = "docs/architecture/PROJECT_CONTROLS_FORECAST_MODEL.md";
...
const FORECAST_ROUTE = "apps/web/src/app/api/engineering/project-controls/forecast/route.ts";
...
const FORECAST_TEST = `${PC}/tests/phase11g-forecast-intelligence.test.ts`;

const FORECAST_DOMAIN_FILES = [
  `${PC}/src/domain/forecast.ts`,
  `${PC}/src/domain/forecast-confidence.ts`,
  `${PC}/src/domain/forecast-engine.ts`,
...
type GateResult = { id: Phase11gGateId; name: string; status: GateStatus; detail?: string };
...
async function main() {
  ...
  const push = (id: Phase11gGateId, name: string, status: GateStatus, detail?: string) =>
  ...
  const FORECAST = `${PC}/src/domain/forecast.ts`;
  const FORECAST_ENGINE = `${PC}/src/domain/forecast-engine.ts`;
  const FORECAST_CONFIDENCE = `${PC}/src/domain/forecast-confidence.ts`;
  ...
  const test11g = run(
    `pnpm --filter @rtb/project-controls exec -- vitest run tests/phase11g-forecast-intelligence.test.ts`,
  );
  const test11f = run(
  ...
  const arch = run(
    "pnpm --filter @rtb/platform-certification exec -- vitest run src/phase11g-project-controls-forecast.test.ts src/phase11e-project-controls-cost.test.ts",
  );

  const forbidLocksOk = PHASE_11F_FORBIDDEN_CAPABILITIES.every((lock) =>
    has(VERSION, new RegExp(`${lock} = false`)),
  );
  const requiredFlagsOk = PHASE_11G_REQUIRED_CAPABILITIES.every((flag) =>
  ...
  push(
    ...
    exists("pnpm-workspace.yaml") &&
      ...
      exists(SPD_PKG) &&
      exists(PC_CERT_PKG) &&
      has(VERSION, /PROJECT_CONTROLS_VERSION = "0\.7\.0-forecast-intelligence"/) &&
      has(VERSION, /PROJECT_CONTROLS_STATUS = "forecast_intelligence"/) &&
      has(VERSION, /PROJECT_CONTROLS_PHASE = "11G"/) &&
      has(
        VERSION,
  ...
  push(
    ...
    has(
      ...
      has(OWNERSHIP_LOCK, /forecastIntelligenceOwnership/) &&
      has(OWNERSHIP_LOCK, /projectContextCompositionOwnership/) &&
      has(VERSION, /PROJECT_CONTEXT_COMPOSITION_READY = true/) &&
      has(
        VERSION,
        new RegExp(`FORECAST_INTELLIGENCE_OWNERSHIP = "${PHASE_11G_FORECAST_INTELLIGENCE_OWNER}"`),
      ) &&
      has(
        VERSION,
        new RegExp(
  ...
  push(
    ...
    exists(DOC_PRODUCTIVITY_MODEL) &&
      has(DOC_PRODUCTIVITY_MODEL, /Forecast Evidence/) &&
      has(DOC_PRODUCTIVITY_MODEL, /Forecast Control Context/) &&
      has(DOC_PRODUCTIVITY_MODEL, /Forecast Posture/) &&
      has(DOC_PRODUCTIVITY_MODEL, /Composition Layer/) &&
      has(DOC_PRODUCTIVITY_MODEL, /Forecast Confidence/) &&
      has(FORECAST, /export type ForecastEvidence/) &&
      has(FORECAST, /export type ForecastControlContext/) &&
      has(FORECAST, /export type ForecastAssessmentState/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    exists(COMPOSITION) &&
      ...
      has(VERSION, /PROJECT_CONTEXT_COMPOSITION_READY = true/) &&
      has(DOC_PRODUCTIVITY, /composition|composed contributors/i)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(FORECAST, /export type ForecastControlContext/) &&
      has(FORECAST, /trajectoryUnitId/) &&
      has(FORECAST_ENGINE, /controlContext/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(FORECAST_ENGINE, /ForecastIntelligenceEngine/) &&
      has(FORECAST_ENGINE, /forecast_intelligence_advisory_v1/) &&
      has(FORECAST_ENGINE, /abstained/) &&
  ...
  push(
    "P",
    "Forecast Intelligence State",
    has(FORECAST, /export type ForecastAssessmentState/) &&
      has(FORECAST, /assessmentClass/) &&
      has(FORECAST, /forecastPosture/) &&
      has(FORECAST, /contributingContributors/) &&
      has(FORECAST, /advisoryOnly: true/)
      ? "pass"
      : "fail",
  );
  push(
    "Q",
    "Forecast posture governance",
    has(FORECAST, /FORECAST_POSTURES/) &&
      has(FORECAST, /favourable/) &&
      has(FORECAST, /deteriorating/) &&
      has(FORECAST, /isAbstainingForecastSufficiency/) &&
      has(FORECAST_ENGINE, /forecastPosture: abstained \? "unknown"/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(COMPOSITION, /ProjectContextCompositionEngine/) &&
      has(FORECAST, /FORECAST_CONTRIBUTOR_KEYS/) &&
      has(FORECAST, /export type ForecastContributorRef/) &&
      has(FORECAST_ENGINE, /composed\.contributorRefs/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    "Forecast Evidence",
    has(FORECAST, /export type ForecastEvidence/) &&
      has(FORECAST, /sourceType/) &&
      has(FORECAST, /sourceRef/) &&
      has(FORECAST, /provenance/) &&
      has(FORECAST, /completionDateClaimed: false/) &&
      has(FORECAST, /costForecastClaimed: false/) &&
      has(FORECAST, /cpmDerived: false/) &&
      !has(FORECAST, /evidencePayload|payloadCopy/)
      ? "pass"
      : "fail",
  );
  push(
    "T",
    "Forecast Confidence",
    has(FORECAST, /"sufficient"/) &&
      has(FORECAST, /"limited"/) &&
      has(FORECAST, /"insufficient"/) &&
      has(FORECAST, /"conflicting"/) &&
      has(FORECAST, /"stale"/) &&
      has(FORECAST_CONFIDENCE, /createForecastConfidenceEngine|ForecastConfidenceEngine/) &&
      has(FORECAST, /completionDateClaimed: false/) &&
      has(VERSION, /FORECAST_CONFIDENCE_ENGINE_READY = true/)
      ? "pass"
      : "fail",
  );
  push(
    "U",
    "Trajectory unit governance",
    has(FORECAST, /trajectoryUnitId/) &&
      has(FORECAST_ENGINE, /trajectory_unit_id_required/) &&
      has(BATCH_67, /trajectory_unit_id/) &&
      has(FORECAST_ROUTE, /missing_trajectory_unit/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(FORECAST_ENGINE, /asOf/) &&
      has(FORECAST, /assessedAt/) &&
      has(BATCH_67, /assessed_at/) &&
      has(BATCH_67, /recorded_at/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    "Contributor refs",
    has(FORECAST, /FORECAST_CONTRIBUTOR_KEYS/) &&
      has(FORECAST, /ForecastContributorRef/) &&
      has(FORECAST, /contributorKey/) &&
      has(FORECAST, /limitations/)
      ? "pass"
      : "fail",
  );
  ...
  push(
    ...
    has(CONTEXT, /ProjectContextEngine/) &&
      has(CONTEXT, /key: "forecast"/) &&
      has(CONTEXT, /status: "active"/) &&
      has(CONTEXT, /ForecastAssessmentState/) &&
  ...
  push(
    ...
    exists(PROFILE_ROUTE) &&
      has(PROFILE_ROUTE, /forecastIntelligenceReady:\s*true/) &&
      has(PROFILE_ROUTE, /"forecast"/) &&
      has(PROFILE_ROUTE, /projectContextCompositionReady:\s*true/) &&
      has(BATCH_67, /forecast_summary/) &&
      has(POSTGRES, /forecast_summary/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(BATCH_67, /forecast_state_ids/) &&
      has(POSTGRES, /forecast_state_ids/) &&
      has(`${PC}/src/domain/change.ts`, /forecastStateIds/) &&
  ...
  push(
    "AB",
    "Governed review",
    has(REVIEW, /project_controls\.forecast_review/) &&
      has(REVIEW, /FORECAST_REVIEW_WORKFLOW/) &&
      has(REVIEW, /EngineeringWorkflowDefinition/) &&
      has(VERSION, /FORECAST_REVIEW_WORKFLOW_READY = true/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(REVIEW, /forecast_self_approval_forbidden/) &&
      has(REVIEW, /forecast_publish_requires_approved_review/) &&
      has(REVIEW, /forecast_assessment_approval_is_not_completion_date_prediction/) &&
      has(VERSION, /AI_MAY_PUBLISH_FORECAST_FORBIDDEN = true/) &&
      has(VERSION, /AUTONOMOUS_FORECAST_PUBLICATION_ALLOWED = false/) &&
      has(ROLES, /SELF_APPROVE_FORBIDDEN_CAPABILITIES/) &&
      has(FORECAST_ROUTE, /self_approval_forbidden/)
      ? "pass"
      : "fail",
  );
  ...
  push(
    ...
    "Hosted migration",
    exists(BATCH_67) &&
      PHASE_11G_PROJECT_CONTROLS_FORECAST_TABLES.every((table) =>
        has(BATCH_67, new RegExp(`CREATE TABLE IF NOT EXISTS ${table}`)),
      ) &&
      has(BATCH_67, /ENABLE ROW LEVEL SECURITY/) &&
      has(BATCH_67, /CREATE POLICY/) &&
      exists(BATCH_62) &&
      exists(BATCH_63) &&
      exists(BATCH_64) &&
      exists(BATCH_65) &&
  ...
  push(
    "AE",
    "Hosted persistence",
    PHASE_11G_PROJECT_CONTROLS_FORECAST_TABLES.every((table) => has(POSTGRES, new RegExp(table))) &&
      has(PERSISTENCE, /forecastStates/) &&
      has(PERSISTENCE, /forecastEvidence/) &&
      has(PERSISTENCE, /forecastReviews/) &&
      has(PERSISTENCE, /forecastConfidence/) &&
      has(`${PC}/src/domain/repository-factory.ts`, /createProjectControlsRepository/) &&
      has(VERSION, /FORECAST_PERSISTENCE_READY = true/) &&
      hostedAvailable &&
      hosted.tablesOk
      ? "pass"
      : "fail",
  ...
  push(
    "AF",
    "Predictive ForecastProvider reserved",
    has(PROVIDERS, /ForecastProvider/) &&
      has(VERSION, /PREDICTIVE_SCHEDULING_IMPLEMENTED = false/) &&
      has(VERSION, /FORECAST_EXECUTION_IMPLEMENTED = false/) &&
      has(FORECAST_ENGINE, /predictiveSchedulingPerformed: false/) &&
      has(FORECAST, /completionDatePredicted: false/)
      ? "pass"
      : "fail",
  );
  push(
  ...
  push(
    ...
    has(VERSION, /RESOURCE_PLANNING_IMPLEMENTED = false/) &&
      has(FORECAST_ENGINE, /resourcePlanningPerformed: false/) &&
      has(FORECAST_ENGINE, /assertNoPredictiveScheduling/)
      ? "pass"
      : "fail",
  );
  ...
  push(
    ...
    has(FORECAST_ENGINE, /assertNoEarnedValueOrCpm/) &&
      has(FORECAST_ENGINE, /earnedValueImplemented: false/) &&
      has(BATCH_67, /pc_fc_no_earned_value/) &&
  ...
  push(
    ...
    has(FORECAST_ENGINE, /assertNoEarnedValueOrCpm/) &&
      ...
      has(VERSION, /FORECAST_ENGINE_IMPLEMENTED = false/) &&
      has(BATCH_67, /pc_fc_no_predictive_scheduling/) &&
  ...
  push(
    ...
    has(FORECAST, /completionDatePredicted: false/) &&
      has(FORECAST_ENGINE, /completionDatePredicted: false/) &&
      has(BATCH_67, /pc_fc_no_completion_date/) &&
      has(FORECAST_ROUTE, /completionDateClaimed/)
      ? "pass"
      : "fail",
  );
  push(
    "AM",
    "Cost forecast forbidden",
    has(FORECAST, /costForecastComputed: false/) &&
      has(VERSION, /FORECASTING_IMPLEMENTED = false/) &&
      has(BATCH_67, /pc_fc_no_cost_forecast/) &&
      has(FORECAST_ROUTE, /costForecastClaimed/)
      ? "pass"
      : "fail",
  );

  // ------------------------------------------------------------------ AN–AW
  const eventsOk = PHASE_11G_FORECAST_EVENTS.every((event) =>
  ...
  push(
    ...
    eventsOk &&
      PHASE_11G_FORECAST_EVENTS.every((event) =>
        has(BATCH_67, new RegExp(event.replace(/\./g, "\\."))),
      ) &&
      has(POSTGRES, /project_controls_outbox_events/) &&
      has(EVENTS, /forecastEventPayload/) &&
  ...
  push(
    "AO",
    "HTTP contracts",
    exists(FORECAST_ROUTE) &&
      has(FORECAST_ROUTE, /forecastIntelligenceReady:\s*true/) &&
      has(FORECAST_ROUTE, /projectContextCompositionReady:\s*true/) &&
      has(FORECAST_ROUTE, /resourcePlanningImplemented:\s*false/) &&
      has(FORECAST_ROUTE, /forecastExecutionImplemented:\s*false/) &&
      has(FORECAST_ROUTE, /predictiveSchedulingImplemented:\s*false/) &&
      has(FORECAST_ROUTE, /phase11hReady:\s*true/) &&
      has(FORECAST_ROUTE, /assess_forecast/) &&
      has(FORECAST_ROUTE, /error:\s*\{\s*code/) &&
      has(FORECAST_ROUTE, /requestId/)
      ? "pass"
      : "fail",
  );
  push(
    "AP",
    "JWT role matrix",
    has(ROLES, /"forecast\.read"/) &&
      has(ROLES, /"forecast\.assess"/) &&
      has(ROLES, /"forecast\.review"/) &&
      has(ROLES, /"forecast\.publish"/) &&
  ...
  push(
    "AQ",
    "Tenant isolation",
    has(BATCH_67, /tenant_id/) &&
      has(BATCH_67, /get_user_tenant_ids/) &&
  ...
  push(
    "AR",
    "Workspace isolation",
    has(BATCH_67, /workspace_id/) &&
      has(BATCH_67, /workspace_memberships/) &&
  ...
  push(
    "AS",
    "IDOR",
    has(FORECAST_ENGINE, /scope_project_mismatch/) &&
      has(FORECAST_ROUTE, /missing_scope/) &&
      has(FORECAST_ENGINE, /project_id_required/) &&
      has(`${PC}/src/domain/engine.ts`, /assessForecast/)
      ? "pass"
      : "fail",
  );
  push(
  ...
  push(
    ...
    has(PERSISTENCE, /optimistic_lock_conflict/) &&
      ...
      has(BATCH_67, /UNIQUE \(tenant_id, workspace_id, project_id/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    "Observability",
    has(FORECAST_ROUTE, /requestId/) &&
      has(FORECAST_ROUTE, /correlationId/) &&
      has(FORECAST_ROUTE, /durationMs/) &&
      has(FORECAST_ROUTE, /x-request-id/) &&
      has(FORECAST_ROUTE, /x-correlation-id/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    "Performance",
    has(FORECAST_ROUTE, /durationMs/) &&
      has(FORECAST_ROUTE, /started = Date\.now/) &&
      test11g.ok && test11f.ok
      ? "pass"
      : "fail",
    `${test11g.detail ?? ""};${test11f.detail ?? ""}`,
  );
  ...
  push(
    ...
    has(VERSION, /CPM_SCHEDULING_IMPLEMENTED = false/) &&
      ...
      has(BATCH_67, /pc_fc_no_cpm/) &&
      has(BATCH_67, /pc_fc_no_float/) &&
  ...
  push(
    ...
    has(VERSION, /SCHEDULE_EXECUTION_IMPLEMENTED = false/) &&
      has(BATCH_67, /pc_fc_no_schedule_execution/) &&
      has(FORECAST, /scheduleExecuted: false/)
      ? "pass"
      : "fail",
  );
  push(
  ...
  const forecastIntelligenceReady =
    priorGatesPassed &&
    hostedOk &&
    requiredFlagsOk &&
    has(CONTEXT, /key: "forecast"/) &&
  ...
  const releaseEligible =
    forecastIntelligenceReady &&
    aiSurfaceUnchanged &&
    piSurfaceUnchanged &&
    iiSurfaceUnchanged &&
    batch61To66Unchanged &&
    has(PC_PKG, /"version": "0\.7\.0-forecast-intelligence"/) &&
    has(PC_CERT_PKG, /"version": "0\.7\.0-forecast-intelligence"/) &&
    has(VERSION, /PROGRESS_INTELLIGENCE_11B_INTACT = true/) &&
    has(VERSION, /CHANGE_INTELLIGENCE_11D_INTACT = true/) &&
    has(VERSION, /SCHEDULE_INTELLIGENCE_11C_INTACT = true/) &&
    has(VERSION, /PRODUCTIVITY_INTELLIGENCE_11F_INTACT = true/) &&
  ...
  const phase11hReady = has(VERSION, /PHASE_11H_READY = true/);

  push(
    "BC",
    "Phase 11H readiness",
    phase11hReady && forecastIntelligenceReady ? "pass" : "fail",
    `phase11hReady=${phase11hReady};forecastReady=${forecastIntelligenceReady};hosted=${hostedAvailable}`,
  );
  ...
  const artifact = {
    schemaVersion: "phase11g-project-controls-forecast/1",
    phase: "11G",
    title: "Project Controls Forecast Intelligence",
    moduleKey: "project_controls",
    version: PHASE_11G_PROJECT_CONTROLS_VERSION,
    status: "forecast_intelligence",
    ...
    forecastIntelligenceOwnership: PHASE_11G_FORECAST_INTELLIGENCE_OWNER,
    ...
    autonomousForecastPublicationAllowed: false,
    ...
    forecastTables: [...PHASE_11G_PROJECT_CONTROLS_FORECAST_TABLES],
    ...
    forecastEvents: [...PHASE_11G_FORECAST_EVENTS],
    phase11fReady: true,
    phase11gReady: true,
    phase11hReady: phase11hReady,
    releaseEligible: pass && releaseEligible,
    requiredTestsSkipped: false,
    unexpected5xx: false,
    gates,
    requiredGates: PHASE_11G_PROJECT_CONTROLS_FORECAST_GATES.map(([id]) => id),
    gateCount: PHASE_11G_GATE_COUNT,
  ...
  const outPath = resolve(outDir, "phase11g-project-controls-forecast-certification.json");
