...
import {
  ...
  PHASE_11I_PROJECT_CONTROLS_SCENARIO_GATES,
  PHASE_11I_PROJECT_CONTROLS_SCENARIO_TABLES,
  PHASE_11I_PROJECT_CONTROLS_DECISION_TABLES,
...
const RUNNER_FILE = `${PC_CERT}/scripts/run-phase11i-certification.ts`;
const SECRET_SCAN_FILE = `${PC_CERT}/scripts/secret-exposure-scan.ts`;
const ARCH_TEST = "packages/platform-certification/src/phase11i-project-controls-scenario.test.ts";
const WORKFLOW = ".github/workflows/phase-11i-project-controls-scenario.yml";
...
const DOC_SCENARIO = "docs/architecture/PROJECT_CONTROLS_SCENARIO_INTELLIGENCE.md";
const DOC_SCENARIO_MODEL = "docs/architecture/PROJECT_CONTROLS_SCENARIO_MODEL.md";
...
const SCENARIO_DOMAIN_FILES = [
  `${PC}/src/domain/scenario.ts`,
  `${PC}/src/domain/scenario-confidence.ts`,
  `${PC}/src/domain/scenario-engine.ts`,
...
type GateResult = { id: Phase11iGateId; name: string; status: GateStatus; detail?: string };
    ...
    if (!createErr && created.user) {
      ...
      if (!signErr && signed.session?.access_token) {
        ...
        jwtMatrixOk =
          ...
          has(`${PC}/src/domain/role-matrix.ts`, /scenario\.assess/) &&
          has(`${PC}/src/domain/review-workflow.ts`, /scenario_self_approval_forbidden/);
      }
...
async function main() {
  ...
  const push = (id: Phase11iGateId, name: string, status: GateStatus, detail?: string) =>
  ...
  const SCENARIO = `${PC}/src/domain/scenario.ts`;
  const SCENARIO_ENGINE = `${PC}/src/domain/scenario-engine.ts`;
  const SCENARIO_CONFIDENCE = `${PC}/src/domain/scenario-confidence.ts`;
  ...
  const arch = run(
    "pnpm --filter @rtb/platform-certification exec -- vitest run src/phase11i-project-controls-scenario.test.ts src/phase11e-project-controls-cost.test.ts",
  );

  const forbidLocksOk = PHASE_11F_FORBIDDEN_CAPABILITIES.every((lock) =>
    has(VERSION, new RegExp(`${lock} = false`)),
  );
  const requiredFlagsOk = PHASE_11I_REQUIRED_CAPABILITIES.every((flag) =>
  ...
  push(
    ...
    exists("pnpm-workspace.yaml") &&
      ...
      exists(SPD_PKG) &&
      exists(PC_CERT_PKG) &&
      has(VERSION, /PROJECT_CONTROLS_VERSION = "0\.9\.0-scenario-intelligence"/) &&
      has(VERSION, /PROJECT_CONTROLS_STATUS = "scenario_intelligence"/) &&
      has(VERSION, /PROJECT_CONTROLS_PHASE = "11I"/) &&
      has(
        VERSION,
  ...
  push(
    ...
    has(
      ...
      has(OWNERSHIP_LOCK, /scenarioIntelligenceOwnership/) &&
      ...
      has(
        VERSION,
        new RegExp(`scenarioIntelligenceOwnership = "${PHASE_11I_SCENARIO_INTELLIGENCE_OWNER}"`),
      ) &&
      has(
        VERSION,
        new RegExp(
  ...
  push(
    ...
    exists(DOC_SCENARIO_MODEL) &&
      has(DOC_SCENARIO_MODEL, /Scenario Evidence/) &&
      has(DOC_SCENARIO_MODEL, /Scenario Control Context/) &&
      has(DOC_SCENARIO_MODEL, /Scenario Confidence/) &&
      has(SCENARIO, /export type ScenarioEvidence/) &&
      has(SCENARIO, /export type ScenarioControlContext/) &&
      has(SCENARIO, /export type ScenarioAssessmentState/)
      ? "pass"
      : "fail",
  );
  push(
  ...
  push(
    ...
    has(SCENARIO_ENGINE, /ProjectControlsScenarioIntelligenceEngine|ScenarioIntelligenceEngine/) &&
      has(SCENARIO_ENGINE, /scenario_intelligence_advisory_v1/) &&
  ...
  push(
    "P",
    "Scenario Intelligence State",
    has(SCENARIO, /export type ScenarioAssessmentState/) &&
      has(SCENARIO, /assessmentClass/) &&
      has(SCENARIO, /comparison/) &&
  ...
  push(
    "Q",
    "Scenario taxonomy governance",
    has(SCENARIO, /SCENARIO_TYPES/) &&
      has(SCENARIO, /maintain_current_posture/) &&
      has(SCENARIO, /investigate/) &&
      has(SCENARIO, /FORBIDDEN_SCENARIO_SELECTION/) &&
      has(SCENARIO, /isAbstainingScenarioSufficiency/) &&
      has(SCENARIO_ENGINE, /abstained/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(COMPOSITION, /ProjectContextCompositionEngine/) &&
      has(SCENARIO, /SCENARIO_CONTRIBUTOR_KEYS/) &&
      has(SCENARIO, /export type ScenarioContributorRef/) &&
  ...
  push(
    ...
    has(SCENARIO, /"sufficient"/) &&
      ...
      has(SCENARIO, /"stale"/) &&
      has(SCENARIO_CONFIDENCE, /createScenarioConfidenceEngine|ScenarioConfidenceEngine/) &&
      has(SCENARIO, /autoExecutionClaimed: false/) &&
      has(SCENARIO, /approvalAuthorityClaimed: false/) &&
      has(VERSION, /SCENARIO_CONFIDENCE_ENGINE_READY = true/)
      ? "pass"
      : "fail",
  );
  push(
    "U",
    "Scenario unit governance",
    has(SCENARIO, /scenarioUnitId/) &&
      has(SCENARIO_ENGINE, /scenario_unit_id_required/) &&
      has(BATCH_69, /scenario_unit_id/) &&
      has(SCENARIO_ROUTE, /missing_scenario_unit/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(SCENARIO_ENGINE, /asOf/) &&
      ...
      has(BATCH_69, /assessed_at/) &&
      has(BATCH_69, /recorded_at/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    "Contributor refs",
    has(SCENARIO, /SCENARIO_CONTRIBUTOR_KEYS/) &&
      has(SCENARIO, /ScenarioContributorRef/) &&
  ...
  push(
    ...
    has(CONTEXT, /ProjectContextEngine/) &&
      has(CONTEXT, /key: "scenario_intelligence"/) &&
      has(CONTEXT, /status: "active"/) &&
      has(CONTEXT, /ScenarioAssessmentState/) &&
  ...
  push(
    ...
    exists(PROFILE_ROUTE) &&
      ...
      has(PROFILE_ROUTE, /"scenario_intelligence"/) &&
      has(PROFILE_ROUTE, /projectContextCompositionReady:\s*true/) &&
      has(BATCH_69, /scenario_summary/) &&
      has(POSTGRES, /scenario_summary/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(BATCH_69, /scenario_state_ids/) &&
      has(POSTGRES, /scenario_state_ids/) &&
      has(`${PC}/src/domain/change.ts`, /scenarioStateIds/) &&
  ...
  push(
    "AB",
    "Governed review",
    has(REVIEW, /project_controls\.scenario_review/) &&
      has(REVIEW, /SCENARIO_REVIEW_WORKFLOW/) &&
      has(REVIEW, /EngineeringWorkflowDefinition/) &&
      has(VERSION, /SCENARIO_REVIEW_WORKFLOW_READY = true/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(REVIEW, /scenario_self_approval_forbidden/) &&
      has(REVIEW, /scenario_publish_requires_approved_review/) &&
      has(REVIEW, /scenario_assessment_approval_is_not_project_or_contract_approval/) &&
      has(VERSION, /AI_MAY_PUBLISH_SCENARIO_FORBIDDEN = true/) &&
      has(VERSION, /AUTONOMOUS_SCENARIO_PUBLICATION_ALLOWED = false/) &&
      has(ROLES, /SELF_APPROVE_FORBIDDEN_CAPABILITIES/) &&
      has(SCENARIO_ROUTE, /self_approval_forbidden/)
      ? "pass"
      : "fail",
  );
  ...
  push(
    ...
    "Hosted migration",
    exists(BATCH_69) &&
      PHASE_11I_PROJECT_CONTROLS_SCENARIO_TABLES.every((table) =>
        has(BATCH_69, new RegExp(`CREATE TABLE IF NOT EXISTS ${table}`)),
      ) &&
      has(BATCH_69, /ENABLE ROW LEVEL SECURITY/) &&
      has(BATCH_69, /CREATE POLICY/) &&
      exists(BATCH_62) &&
      exists(BATCH_63) &&
      exists(BATCH_64) &&
      exists(BATCH_65) &&
  ...
  push(
    "AE",
    "Hosted persistence",
    PHASE_11I_PROJECT_CONTROLS_SCENARIO_TABLES.every((table) => has(POSTGRES, new RegExp(table))) &&
      has(PERSISTENCE, /scenarioStates/) &&
      has(PERSISTENCE, /scenarioEvidence/) &&
      has(PERSISTENCE, /scenarioReviews/) &&
      has(PERSISTENCE, /scenarioConfidence/) &&
      has(`${PC}/src/domain/repository-factory.ts`, /createProjectControlsRepository/) &&
      has(VERSION, /SCENARIO_PERSISTENCE_READY = true/) &&
      hostedAvailable &&
      hosted.tablesOk
      ? "pass"
      : "fail",
  ...
  push(
    ...
    has(PROVIDERS, /DecisionProvider/) &&
      ...
      has(SCENARIO, /completionDatePredicted: false/)
      ? "pass"
      : "fail",
  );
  push(
  ...
  push(
    ...
    has(SCENARIO_ENGINE, /assertNoEarnedValueOrCpm/) &&
      ...
      has(BATCH_69, /pc_sc_no_earned_value/) &&
  ...
  push(
    ...
    has(SCENARIO_ENGINE, /assertNoEarnedValueOrCpm/) &&
      ...
      has(VERSION, /DECISION_ENGINE_IMPLEMENTED = false/) &&
      has(BATCH_69, /pc_sc_no_predictive_scheduling/) &&
  ...
  push(
    ...
    has(SCENARIO, /completionDatePredicted: false/) &&
      has(SCENARIO_ENGINE, /completionDatePredicted: false/) &&
      has(BATCH_69, /pc_sc_no_auto_execution/) &&
      has(SCENARIO_ROUTE, /completionDateClaimed/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    has(SCENARIO, /costDecisionComputed: false/) &&
      has(VERSION, /DECISIONING_IMPLEMENTED = false/) &&
      has(BATCH_69, /pc_sc_no_contract_instruction/) &&
      has(SCENARIO_ROUTE, /costDecisionClaimed/)
      ? "pass"
      : "fail",
  );

  // ------------------------------------------------------------------ AN–AW
  const eventsOk = PHASE_11I_SCENARIO_EVENTS.every((event) =>
  ...
  push(
    ...
    eventsOk &&
      PHASE_11I_SCENARIO_EVENTS.every((event) =>
        has(BATCH_69, new RegExp(event.replace(/\./g, "\\."))),
      ) &&
      has(POSTGRES, /project_controls_outbox_events/) &&
      has(EVENTS, /scenarioEventPayload/) &&
  ...
  push(
    "AO",
    "HTTP contracts",
    exists(SCENARIO_ROUTE) &&
      has(SCENARIO_ROUTE, /scenarioIntelligenceReady:\s*true/) &&
      has(SCENARIO_ROUTE, /projectContextCompositionReady:\s*true/) &&
      has(SCENARIO_ROUTE, /resourcePlanningImplemented:\s*false/) &&
      has(SCENARIO_ROUTE, /decisionExecutionImplemented:\s*false/) &&
      has(SCENARIO_ROUTE, /automaticScenarioExecutionEnabled:\s*false/) &&
      has(SCENARIO_ROUTE, /predictiveSchedulingImplemented:\s*false/) &&
      has(SCENARIO_ROUTE, /phase11jReady:\s*true/) &&
      has(SCENARIO_ROUTE, /assess_scenario/) &&
      has(SCENARIO_ROUTE, /error:\s*\{\s*code/) &&
      has(SCENARIO_ROUTE, /requestId/)
      ? "pass"
      : "fail",
  );
  push(
    "AP",
    "JWT role matrix",
    has(ROLES, /"scenario\.read"/) &&
      has(ROLES, /"scenario\.assess"/) &&
      has(ROLES, /"scenario\.review"/) &&
      has(ROLES, /"scenario\.publish"/) &&
  ...
  push(
    "AQ",
    "Tenant isolation",
    has(BATCH_69, /tenant_id/) &&
      has(BATCH_69, /get_user_tenant_ids/) &&
  ...
  push(
    "AR",
    "Workspace isolation",
    has(BATCH_69, /workspace_id/) &&
      has(BATCH_69, /workspace_memberships/) &&
  ...
  push(
    ...
    has(SCENARIO_ENGINE, /scope_project_mismatch/) &&
      has(SCENARIO_ROUTE, /missing_scope/) &&
      has(SCENARIO_ENGINE, /project_id_required/) &&
      has(`${PC}/src/domain/engine.ts`, /assessScenario/)
      ? "pass"
      : "fail",
  );
  push(
  ...
  push(
    ...
    has(PERSISTENCE, /optimistic_lock_conflict/) &&
      ...
      has(BATCH_69, /UNIQUE \(tenant_id, workspace_id, project_id/)
      ? "pass"
      : "fail",
  );
  push(
    ...
    "Observability",
    has(SCENARIO_ROUTE, /requestId/) &&
      has(SCENARIO_ROUTE, /correlationId/) &&
      has(SCENARIO_ROUTE, /durationMs/) &&
      has(SCENARIO_ROUTE, /x-request-id/) &&
  ...
  push(
    ...
    has(VERSION, /CPM_SCHEDULING_IMPLEMENTED = false/) &&
      ...
      has(BATCH_69, /pc_sc_no_cpm/) &&
      has(BATCH_69, /pc_sc_no_float/) &&
  ...
  push(
    ...
    has(VERSION, /SCHEDULE_EXECUTION_IMPLEMENTED = false/) &&
      has(BATCH_69, /pc_sc_no_schedule_execution/) &&
      has(SCENARIO, /scheduleExecuted: false/)
      ? "pass"
      : "fail",
  );
  push(
  ...
  const ScenarioIntelligenceReady =
    has(VERSION, /SCENARIO_INTELLIGENCE_READY = true/) &&
    has(CONTEXT, /key: "scenario_intelligence"/) &&
    has(COMPOSITION, /ProjectContextCompositionEngine/) &&
    has(SCENARIO_ENGINE, /ScenarioIntelligenceEngine/);
  const releaseEligible =
    ScenarioIntelligenceReady &&
    aiSurfaceUnchanged &&
    piSurfaceUnchanged &&
    iiSurfaceUnchanged &&
    batch61To66Unchanged &&
    has(PC_PKG, /"version": "0\.9\.0-scenario-intelligence"/) &&
    has(PC_CERT_PKG, /"version": "0\.9\.0-scenario-intelligence"/) &&
    has(VERSION, /PROGRESS_INTELLIGENCE_11B_INTACT = true/) &&
    has(VERSION, /CHANGE_INTELLIGENCE_11D_INTACT = true/) &&
    has(VERSION, /SCHEDULE_INTELLIGENCE_11C_INTACT = true/) &&
    has(VERSION, /PRODUCTIVITY_INTELLIGENCE_11F_INTACT = true/) &&
  ...
  const phase11jReady = has(VERSION, /PHASE_11J_READY = true/);

  push(
    "BC",
    "Phase 11J readiness",
    phase11jReady && has(VERSION, /SCENARIO_INTELLIGENCE_READY = true/) ? "pass" : "fail",
    `phase11jReady=${phase11jReady};scenarioReady=${has(VERSION, /SCENARIO_INTELLIGENCE_READY = true/)};hosted=${hostedAvailable}`,
  );
  ...
  const artifact = {
    schemaVersion: "phase11i-project-controls-scenario/1",
    phase: "11I",
    title: "Project Controls Scenario Intelligence",
    moduleKey: "project_controls",
    version: PHASE_11I_PROJECT_CONTROLS_VERSION,
    status: "scenario_intelligence",
    ...
    ScenarioIntelligenceReady: true,
    scenarioConfidenceEngineReady: true,
    scenarioReviewWorkflowReady: true,
    scenarioPersistenceReady: true,
    ...
    scenarioIntelligenceOwnership: PHASE_11I_SCENARIO_INTELLIGENCE_OWNER,
    ...
    scenarioTables: [...PHASE_11I_PROJECT_CONTROLS_SCENARIO_TABLES],
    decisionTables: [...PHASE_11I_PROJECT_CONTROLS_DECISION_TABLES],
    ...
    scenarioEvents: [...PHASE_11I_SCENARIO_EVENTS],
    phase11hReady: true,
    phase11jReady: phase11jReady,
    releaseEligible: pass && releaseEligible,
    requiredTestsSkipped: false,
    unexpected5xx: false,
    gates,
    requiredGates: PHASE_11I_PROJECT_CONTROLS_SCENARIO_GATES.map(([id]) => id),
    gateCount: PHASE_11I_GATE_COUNT,
  ...
  const outPath = resolve(outDir, "phase11i-project-controls-scenario-certification.json");
