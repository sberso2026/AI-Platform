...
describe("Phase 11E Project Controls ownership and forbid locks", () => {
  it("declares the decision support module identity", () => {
    ...
    expect(PROJECT_CONTROLS_VERSION).toBe("0.10.0-risk-opportunity-intelligence");
    expect(PROJECT_CONTROLS_STATUS).toBe("risk_opportunity_intelligence");
    expect(PROJECT_CONTROLS_PHASE).toBe("11J");
  ...
  it("exposes a coherent declaration", () => {
    ...
    expect(declaration.version).toBe("0.10.0-risk-opportunity-intelligence");
    expect(declaration.status).toBe("risk_opportunity_intelligence");
    expect(declaration.phase).toBe("11J");
    ...
    expect(declaration.hierarchy).toContain(
      "progress + schedule + change + cost + productivity + forecast + decision support + scenario + risk/opportunity intelligence",
    );
