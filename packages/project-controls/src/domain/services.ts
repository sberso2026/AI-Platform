...
/**
 * Exploratory scenario comparison from composed contributors, forecast and decision only.
 */
export class ScenarioIntelligenceService {
  constructor(private readonly engine: ProjectControlsEngine) {}

  assess(command: AssessScenarioCommand): Promise<AssessScenarioResult> {
    return this.engine.assessScenario(command);
  }

  review(command: ReviewScenarioCommand): Promise<ReviewScenarioResult> {
    return this.engine.reviewScenario(command);
  }
  ...
  latest(input: {
    ...
    actorRole: ProjectControlsRole;
    asOf?: string;
  }): Promise<ScenarioAssessmentState | undefined> {
    return this.engine.getLatestScenario(input);
  }
  ...
  history(input: {
    ...
    projectId: string;
    actorRole: ProjectControlsRole;
  }): Promise<ScenarioAssessmentState[]> {
    return this.engine.listScenarioHistory(input);
  }
}

export class RiskOpportunityIntelligenceService {
  constructor(private readonly engine: ProjectControlsEngine) {}

  assess(command: AssessRiskOpportunityCommand): Promise<AssessRiskOpportunityResult> {
    return this.engine.assessRiskOpportunity(command);
  }

  review(command: ReviewRiskOpportunityCommand): Promise<ReviewRiskOpportunityResult> {
    return this.engine.reviewRiskOpportunity(command);
  }
  ...
  latest(input: {
    ...
    actorRole: ProjectControlsRole;
    asOf?: string;
  }): Promise<RiskOpportunityAssessmentState | undefined> {
    return this.engine.getLatestRiskOpportunity(input);
  }
  ...
  history(input: {
    ...
    projectId: string;
    actorRole: ProjectControlsRole;
  }): Promise<RiskOpportunityAssessmentState[]> {
    return this.engine.listRiskOpportunityHistory(input);
  }
}
