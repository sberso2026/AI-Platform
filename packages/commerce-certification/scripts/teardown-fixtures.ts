...
function log(msg: string): void {
  console.log(`[commerce:teardown] ${msg}`);
}
...
async function main(): Promise<void> {
  ...
  if (!existsSync(manifestPath)) {
    log("No cert-fixtures.json — nothing to teardown");
    process.exit(0);
  }
...
main().catch((err) => {
  console.error(`[commerce:teardown] FAIL: ${err instanceof Error ? err.message : String(err)}`);
  process.exit(1);
});
