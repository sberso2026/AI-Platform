...
import { isCertificationMode, requireFixturesManifest, type CertFixturesManifest } from "../lib/env.js";
...
export function initRlsSuite(): void {
  const envReady = Boolean(
    process.env.SUPABASE_TEST_URL &&
      ...
      (loadManifestSafe() || !isCertificationMode())
  );

  if (isCertificationMode() && !loadManifestSafe()) {
    throw new Error("COMMERCE_CERTIFICATION=1: artifacts/cert-fixtures.json required");
  }

  beforeAll(() => {
    if (!envReady && !isCertificationMode()) return;
