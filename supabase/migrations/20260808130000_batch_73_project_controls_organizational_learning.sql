-- Phase 11M — Project Controls organizational learning intelligence persistence (batch_73)
--
-- Organizational Learning Intelligence is ADVISORY. It surfaces historical references;
-- it is NOT prediction, recommendation, approval, knowledge mutation, or optimisation.
--
-- Reuses project_controls_project_snapshots + project_controls_project_timeline.
-- Additive only; batch_61–72 not rewritten.

CREATE TABLE IF NOT EXISTS project_controls_organizational_learning_states (
  ...
  organizational_learning_unit_id text NOT NULL,
  organizational_learning_unit_label text,
  ...
  taxonomy_class text NOT NULL
    CHECK (taxonomy_class IN (
      'historical_pattern', 'recurring_issue', 'recurring_success', 'lesson_learned',
      'knowledge_gap', 'best_practice', 'similar_project', 'unknown'
    )),
  synthesis jsonb NOT NULL DEFAULT '{}'::jsonb,
  learning_items jsonb NOT NULL DEFAULT '[]'::jsonb,
  ...
  data_sufficiency text NOT NULL
    CHECK (data_sufficiency IN ('sufficient', 'limited', 'insufficient', 'conflicting', 'incomplete')),
  ...
  method text NOT NULL DEFAULT 'organizational_learning_intelligence_advisory_v1'
    CHECK (method = 'organizational_learning_intelligence_advisory_v1'),
  ...
  supersedes_id uuid REFERENCES project_controls_organizational_learning_states(id) ON DELETE SET NULL,
  ...
  fabricated_lesson boolean NOT NULL DEFAULT false
    CONSTRAINT pc_ol_no_fabricated_lesson CHECK (fabricated_lesson = false),
  unsupported_similarity_score boolean NOT NULL DEFAULT false
    CONSTRAINT pc_ol_no_unsupported_similarity CHECK (unsupported_similarity_score = false),
  knowledge_mutation_claimed boolean NOT NULL DEFAULT false
    CONSTRAINT pc_ol_no_knowledge_mutation CHECK (knowledge_mutation_claimed = false),
  ...
  duplicate_knowledge_ownership_detected boolean NOT NULL DEFAULT false
    CONSTRAINT pc_ol_no_duplicate_knowledge_ownership CHECK (duplicate_knowledge_ownership_detected = false),
  ...
  UNIQUE (tenant_id, workspace_id, project_id, scope_kind, scope_reference_id, organizational_learning_unit_id, version)
);

CREATE INDEX IF NOT EXISTS idx_pc_assurance_states_tenant_ws_project
  ON project_controls_organizational_learning_states(tenant_id, workspace_id, project_id, recorded_at DESC);

CREATE TABLE IF NOT EXISTS project_controls_organizational_learning_evidence (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  workspace_id uuid NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  project_id uuid NOT NULL REFERENCES engineering_projects(id) ON DELETE RESTRICT,
  organizational_learning_state_id uuid NOT NULL REFERENCES project_controls_organizational_learning_states(id) ON DELETE CASCADE,
  ...
  fabricated_lesson boolean NOT NULL DEFAULT false
    CONSTRAINT pc_as_evidence_no_certification CHECK (fabricated_lesson = false),
  unsupported_similarity_score boolean NOT NULL DEFAULT false
    CONSTRAINT pc_as_evidence_no_verification CHECK (unsupported_similarity_score = false),
  knowledge_mutation_claimed boolean NOT NULL DEFAULT false
    CONSTRAINT pc_as_evidence_no_evidence_approval CHECK (knowledge_mutation_claimed = false),
...
CREATE INDEX IF NOT EXISTS idx_pc_organizational_learning_evidence_state
  ON project_controls_organizational_learning_evidence(organizational_learning_state_id);

CREATE TABLE IF NOT EXISTS project_controls_organizational_learning_confidence (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  workspace_id uuid NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  project_id uuid NOT NULL REFERENCES engineering_projects(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS project_controls_organizational_learning_reviews (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  workspace_id uuid NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  project_id uuid NOT NULL REFERENCES engineering_projects(id) ON DELETE RESTRICT,
  ...
  fabricated_lesson boolean NOT NULL DEFAULT false
    CONSTRAINT pc_as_review_no_certification CHECK (fabricated_lesson = false),
  unsupported_similarity_score boolean NOT NULL DEFAULT false
    CONSTRAINT pc_as_review_no_verification CHECK (unsupported_similarity_score = false)
);

ALTER TABLE project_controls_project_snapshots
  ADD COLUMN IF NOT EXISTS organizational_learning_state_ids jsonb NOT NULL DEFAULT '[]'::jsonb;

ALTER TABLE project_controls_project_profiles
  ADD COLUMN IF NOT EXISTS organizational_learning_summary jsonb;
...
ALTER TABLE project_controls_organizational_learning_states ENABLE ROW LEVEL SECURITY;
ALTER TABLE project_controls_organizational_learning_evidence ENABLE ROW LEVEL SECURITY;
ALTER TABLE project_controls_organizational_learning_confidence ENABLE ROW LEVEL SECURITY;
ALTER TABLE project_controls_organizational_learning_reviews ENABLE ROW LEVEL SECURITY;
...
COMMENT ON TABLE project_controls_organizational_learning_states IS
  'Project Controls advisory organizational learning intelligence. Not prediction, recommendation, or approval.';
COMMENT ON TABLE project_controls_organizational_learning_evidence IS
  'Evidence supporting an organizational learning assessment; historical and contributor references only.';
COMMENT ON TABLE project_controls_organizational_learning_confidence IS
  'Qualitative confidence posture for organizational learning synthesis.';
COMMENT ON TABLE project_controls_organizational_learning_reviews IS
  'Human review of an organizational learning assessment. self_approved must remain false.';
