-- EOS-PILOT-1RC: invited users join the inviting tenant instead of bootstrapping a new org.
-- Reuses existing tenant_memberships / workspace_memberships / roles. No new identity tables.
...
BEGIN
  ...
  BEGIN
    v_invite_tenant_id := NULLIF(trim(NEW.raw_user_meta_data->>'invited_tenant_id'), '')::uuid;
  EXCEPTION
    WHEN OTHERS THEN
      v_invite_tenant_id := NULL;
  END;

  IF v_invite_tenant_id IS NOT NULL AND EXISTS (SELECT 1 FROM public.tenants WHERE id = v_invite_tenant_id) THEN
    v_invite_role_slug := COALESCE(NULLIF(trim(NEW.raw_user_meta_data->>'invited_role_slug'), ''), 'member');
    ...
    IF v_invite_role_id IS NOT NULL THEN
      ...
      BEGIN
        v_invite_workspace_id := NULLIF(trim(NEW.raw_user_meta_data->>'invited_workspace_id'), '')::uuid;
      EXCEPTION
        WHEN OTHERS THEN
