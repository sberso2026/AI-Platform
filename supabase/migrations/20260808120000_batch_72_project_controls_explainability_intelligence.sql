-- Phase 11L — Project Controls explainability intelligence persistence (batch_72)
--
...
CREATE TABLE IF NOT EXISTS project_controls_explainability_states (
  ...
  explainability_unit_id text NOT NULL,
  explainability_unit_label text,
  ...
  explanation_status text NOT NULL
    CHECK (explanation_status IN (
      'supported', 'partially_supported', 'unsupported', 'conflicting', 'incomplete', 'unknown'
    )),
  synthesis jsonb NOT NULL DEFAULT '{}'::jsonb,
  contributor_explanations jsonb NOT NULL DEFAULT '[]'::jsonb,
  ...
  method text NOT NULL DEFAULT 'explainability_intelligence_advisory_v1'
    CHECK (method = 'explainability_intelligence_advisory_v1'),
  ...
  supersedes_id uuid REFERENCES project_controls_explainability_states(id) ON DELETE SET NULL,
  ...
  chain_of_thought_exposed boolean NOT NULL DEFAULT false
    CONSTRAINT pc_as_no_certification CHECK (chain_of_thought_exposed = false),
  hidden_reasoning_exposed boolean NOT NULL DEFAULT false
    CONSTRAINT pc_as_no_verification CHECK (hidden_reasoning_exposed = false),
  fabricated_provenance boolean NOT NULL DEFAULT false
    CONSTRAINT pc_as_no_evidence_approval CHECK (fabricated_provenance = false),
  ...
  duplicate_explainability_ownership_detected boolean NOT NULL DEFAULT false
    CONSTRAINT pc_as_no_duplicate_assurance_ownership CHECK (duplicate_explainability_ownership_detected = false),
  ...
  UNIQUE (tenant_id, workspace_id, project_id, scope_kind, scope_reference_id, explainability_unit_id, version)
);

CREATE INDEX IF NOT EXISTS idx_pc_assurance_states_tenant_ws_project
  ON project_controls_explainability_states(tenant_id, workspace_id, project_id, recorded_at DESC);

CREATE TABLE IF NOT EXISTS project_controls_explainability_evidence (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  workspace_id uuid NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  project_id uuid NOT NULL REFERENCES engineering_projects(id) ON DELETE RESTRICT,
  explainability_state_id uuid NOT NULL REFERENCES project_controls_explainability_states(id) ON DELETE CASCADE,
  ...
  chain_of_thought_exposed boolean NOT NULL DEFAULT false
    CONSTRAINT pc_as_evidence_no_certification CHECK (chain_of_thought_exposed = false),
  hidden_reasoning_exposed boolean NOT NULL DEFAULT false
    CONSTRAINT pc_as_evidence_no_verification CHECK (hidden_reasoning_exposed = false),
  fabricated_provenance boolean NOT NULL DEFAULT false
    CONSTRAINT pc_as_evidence_no_evidence_approval CHECK (fabricated_provenance = false),
...
CREATE INDEX IF NOT EXISTS idx_pc_explainability_evidence_state
  ON project_controls_explainability_evidence(explainability_state_id);

CREATE TABLE IF NOT EXISTS project_controls_explainability_confidence (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  workspace_id uuid NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  project_id uuid NOT NULL REFERENCES engineering_projects(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS project_controls_explainability_reviews (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  workspace_id uuid NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  project_id uuid NOT NULL REFERENCES engineering_projects(id) ON DELETE RESTRICT,
  ...
  chain_of_thought_exposed boolean NOT NULL DEFAULT false
    CONSTRAINT pc_as_review_no_certification CHECK (chain_of_thought_exposed = false),
  hidden_reasoning_exposed boolean NOT NULL DEFAULT false
    CONSTRAINT pc_as_review_no_verification CHECK (hidden_reasoning_exposed = false)
);

ALTER TABLE project_controls_project_snapshots
  ADD COLUMN IF NOT EXISTS explainability_state_ids jsonb NOT NULL DEFAULT '[]'::jsonb;

ALTER TABLE project_controls_project_profiles
  ADD COLUMN IF NOT EXISTS explainability_summary jsonb;
...
ALTER TABLE project_controls_explainability_states ENABLE ROW LEVEL SECURITY;
ALTER TABLE project_controls_explainability_evidence ENABLE ROW LEVEL SECURITY;
ALTER TABLE project_controls_explainability_confidence ENABLE ROW LEVEL SECURITY;
ALTER TABLE project_controls_explainability_reviews ENABLE ROW LEVEL SECURITY;
...
COMMENT ON TABLE project_controls_explainability_states IS
  'Project Controls advisory explainability intelligence. Not verification, certification, or approval.';
COMMENT ON TABLE project_controls_explainability_evidence IS
  'Evidence supporting an explainability intelligence assessment; composed contributor references only.';
COMMENT ON TABLE project_controls_explainability_confidence IS
  'Qualitative confidence posture for assurance synthesis.';
COMMENT ON TABLE project_controls_explainability_reviews IS
  'Human review of an explainability intelligence assessment. self_approved must remain false.';
