...
Phase 11M. `packages/project-controls/src/domain/project-context-engine.ts`.
...
profile now, with twelve of fourteen contributors active and two declared
...
| `organizational_learning` | **active** | Advisory organizational learning references; pattern≠prediction, lesson≠recommendation; not knowledge mutation |
...
key, that Phase 11M has exactly twelve active contributors
(`progress_intelligence`, `schedule_intelligence`, `change_intelligence`,
`cost_intelligence`, `productivity_intelligence`, `forecast`, `decision_support`, `scenario_intelligence`, `risk_opportunity_intelligence`, `assurance_intelligence`, `explainability_intelligence`, `organizational_learning`), and that
...
Organizational Learning Intelligence consumes historical evidence, lessons-learned references, and composed contributor context,
producing advisory learning references only — never prediction, recommendation, approval, or knowledge mutation. Pattern ≠ prediction; lesson ≠ recommendation; humans own organizational knowledge.
`PROJECT_CONTEXT_COMPOSITION_READY = true`.

## Profile composition

`compose({ tenantId, workspaceId, projectReference, progress, schedule, change, cost, productivity, forecast, decision, scenario, riskOpportunity, assurance, explainability, organizationalLearning })`.
