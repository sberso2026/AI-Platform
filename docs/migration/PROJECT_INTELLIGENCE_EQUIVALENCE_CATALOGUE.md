...
|---------------|-----------------|---------------|-----------------|---------------|--------------|------------------------|------------------|------------|----------|----------------------|------------------|--------------|--------------------|------------------|-----------------|
| DOC-UPLOAD | Document upload and metadata | `/documents`, upload APIs | upload services | `documents` | upload/auth tests | Upload files into project scope | Document metadata rows | Core (meta) / PI (intel) | — | Upload E2E + RLS | not started | 6C-2 | not_equivalent | Dual ownership until cutover | pending |
| DOC-PROCESS | Document processing | parser APIs | parser/OCR pipeline | processing events | parser tests | Processing completes with quality gates | Parsed pages/chunks | PI | — | Parser gates | not started | 6C-2 | not_equivalent | FastAPI parser external | pending |
| DOC-CHUNK | Chunking and embeddings | indexing workers | embedding batch | `document_chunks` | indexing tests | Chunks searchable | Embeddings present | PI | Platform metering | Indexing smoke | not started | 6C-2 | not_equivalent | Worker topology | pending |
| AI-GROUND | Grounded query | Thor chat | retrieval + LLM | AI answers | thor tests | Grounded answers with citations | Answer + citations + confidence | PI | AI Director target | Citation integrity | not started | 6C-3 | not_equivalent | Thor→Director adapter | pending |
| AI-CITE | Citations | chat UI | evidence pack | evidence tables | evidence tests | Citations resolve to sources | Citation list | PI | AI Director | Evidence integrity tests | not started | 6C-3 | not_equivalent | — | pending |
| AI-ABSTAIN | Abstention | Thor guards | confidence gates | — | abstention tests | Abstain when insufficient | Abstain payload | PI | AI Director | Phase 6B proof exists | foundation only | 6B/6C-3 | partial | Platform proof only | pending |
